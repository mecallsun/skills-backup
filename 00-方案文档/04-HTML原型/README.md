# HTML 原型 — v2.12.3（共用页头 + Tab 页签切换 + 系统集成 + 筛选条件持久化缓存 + 7 张 KPI + 考勤班次 + 筛选优化 + 住宿状态 + 表头顺序 + 看板布局重构 + 高度拉伸 + 员工分摊精确计算 + 宿舍账单筛选 + 数据看板月度选择 + 入住段拆分两列 + 入住率排名 TOP 15（%） + 抄表"未抄表"状态 + 抄表"未完成"状态 + **统一 UI 导航栏** + **修改办理登记数据关联可视化** + **KPI 入住人数/异常人员 数据逻辑修复 v2.11.6**）
