// 验证 v2.12.5 变更：住宿状态→在宿状态、新增宿舍房号列
...
console.log('v2.12.5 Verify: 在宿状态 + 宿舍（房号）');
console.log('='.repeat(70));
...
if (tbodyMatch) {
    ...
    rows.slice(0, 10).forEach((row, i) => {
        ...
        // 第 9 列是在职状态，第 10 列是离职日期，第 11 列是在宿状态，第 12 列是宿舍
        console.log(`  [${i + 1}] ${cleaned[2]}(${cleaned[1]}) 在职=${cleaned[8]} 在宿=${cleaned[10]} 宿舍=${cleaned[11]}`);
    });
}
...
labels.forEach(l => {
    ...
    if (text.includes('姓名') || text.includes('在宿') || text.includes('宿舍')) {
