...
| **4** | **数据库参数在线修改（按需）** | 用户 → 托盘菜单/Web 后台 → 「系统配置」/「数据库连接」 | 修改 Provider/Server/Port/Database/User/Password/SqlitePath → 保存时强制测试连通性 → 通过则保存→ `AppConfigManager` 将配置 **AES-256 加密** 后写入 `db_setting.json`，并同步写入 `SysParameter` 表 | 保存前若未通过连通性校验，拒绝保存并提示错误；双 UI 共享同一事实来源 |
