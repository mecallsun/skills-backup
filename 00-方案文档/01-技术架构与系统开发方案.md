...
> - **v2.13.54（2026-07-21）— profile/index.html 补全 mountTabBar 调用**：初次创建时遗漏 `mountTabBar({ basePath: '..', currentUrl: 'profile/index.html' })`，导致 `<div id="tab-bar"></div>` 占位元素保持为空；详见 `106-Profile导航过时描述深度清理-v2.13.53.md §八`
> - **v2.13.55（2026-07-21）— profile/index.html 补全 storage-keys.js 依赖**：tab-bar.js 依赖 STORAGE_KEYS + getCurrentUserId()（来自 storage-keys.js），profile/index.html 是 26 个原型中唯一遗漏此依赖的页面，导致 TabManager.saveActiveId() 抛 ReferenceError → mountTabBar() 中断；详见 `00-方案文档/107-Profile原型JS依赖链缺失修复-v2.13.55.md`
> - **v2.13.56（2026-07-21）— Profile 禁止作为第 11 主菜单原则（硬约束）**：❌ 禁止修改 `tab-bar.js` FIXED_TABS 数组新增 `tab-profile`；❌ 禁止在 Profile 页面隐藏其他 10 个 Tab；✅ Profile 仅通过顶部「用户胶囊」进入；详见 `00-方案文档/108-Profile禁止作为第11主菜单原则-v2.13.56.md`
> - **v2.13.57（2026-07-21）— 住宿登记表 + 人员清单表 DDL 缺失 P0 修复**：`01_DDL_Schema.sql`（253 行）原只有 9 张表，缺失 DormBooking + SysEmployee，导致 SQL Server 运维执行 DDL 后 EF Core 查询 → Invalid object name → 页面显示 Error；新增 2 张表 DDL + FK_DormBooking_Employee + FK_DormBooking_Dorm + 20 条种子数据；详见 `00-方案文档/109-住宿登记表缺失SQL修复-v2.13.57.md`
> - **v2.13.58（2026-07-21）— Seed 数据 DormCode 不匹配 FK 约束修复**：v2.13.57 Seed 用 D-001~D-005，但 Dorm 表种子是 D-301~D-402，FK 约束导致脚本崩溃；修正所有 DormCode 为 D-301~D-402；详见 `00-方案文档/110-SeedDormCodeFK不匹配修复-v2.13.58.md`
...
> - **v2.12.34（2026-07-13）— Dorm 住宿档案新增床位号字段 BedNumbers**：① Dorm 表新增 `BedNumbers NVARCHAR(500)` 字段；② 按住宿房间容量自动生成床位号，从 1 起至 Capacity 上限为床位号自动生成（如 Capacity=4 → `"1,2,3,4"`）；③ 当 Capacity 变更时自动重新生成（追加或截断）；④ 办理入住时默认预分配床位号 = 该住宿当前在宿人数 + 1；⑤ 新增 §4.3.1 Dorm 表字段 + 床位号自动生成规则 |
...
> - **v2.10（2026-07-09）— 新增「第 19 章 人员清单管理」+「第 20 章 费用管理」：人员清单含部门/员工类型/入职离职/工号/手机号/备注字段 + Excel 批量导入导出；费用管理含 BillingStandard 费用标准（热水/冷水/电单价 + 适用员工类型多选勾选）+ 按月统计 住宿费用清单 + 按月分摊 员工费用清单 + 双清单 Excel 导出；新增 3 张表（SysEmployee/BillingStandard/DormBilling/EmployeeBilling）+ 10+ API 端点 + 3 个 Web 后台页面 + 完整业务规则（在职分摊、适用类型筛选、平均分摊算法、发布控制、生效时间区间校验）**
- **v2.11（2026-07-10）— 新增「0. 系统设计规划背景说明」章节：明确系统面向 800 人规模企业、200+ 间住宿（含 1/2/4/6 人间多种房型）、日常入住 500+ 人的设计目标；更新数据库表结构、索引策略、性能基准与容量规划，确保系统在该规模下稳定运行；新增人员生命周期管理（入职/离职/调宿/预约）、费用标准按员工类型差异化、住宿账单按月生成与分摊等业务能力**
...
| **Web 访问管理前端** | Win/Mac/Linux 浏览器 | 人员/住宿/办理/账单全套管理 | ✅ V1.0 已实现 |
...
本系统面向 **金智新材料** 公司，服务其员工住宿管理全流程。系统设计目标规模如下：

| 维度 | 规模 | 说明 |
|------|------|------|
| **公司总人数** | **800 人** | 含合同工、临时工、外包、实习生、驻场等 5 种员工类型 |
| **住宿房间数** | **200+ 间** | 含 1 人间、2 人间、4 人间、6 人间等多种房型配置 |
| **日常入住人数** | **500+ 人** | 按 800 人总规模的 62.5% 入住率设计 |
| **床位总数** | **≥ 800 个** | 按 1:1 冗余设计，确保高峰期全员可入住 |
| **抄表频率** | **每月 1 次** | 全 200+ 间住宿每月抄表一次，PDA 端现场录入 |
...
```
员工入职 → 分配住宿（预约/立即入住） → 月度抄表（PDA） → 自动生成账单 → 费用分摊 → 发布查看
    ↑                                                                      ↓
...
- **住宿**：200+ 间档案（楼栋/楼层/房型/容量/设施）
- **办理登记**：入住/退房/调宿 全流程留痕
- **智能抄表**：200+ 间 × 12 月 × N 年 = 持续累积
- **费用账单**：每月自动生成住宿账单 + 员工分摊账单
...
| **16** | **DormMeter**（v2.13.120 新增；v2.13.168 设备ID 3 列 filtered unique index） | **设备档案（与 Dorm 1:1；电/冷/热表ID 全局唯一）** | **0** | **0** | **0** |
| **17** | **EquipmentReading**（v2.13.130 新增） | **设备读数日志（三层数据模型日志层）** | **0** | **+7,200（200×12×3）** | **36,000** |
| **合计** | | | **1,003** | **+23,002/年** | **≈ 116,000 行** |
...
**完整闭环**：PDA 扫码 → 匹配住宿 → 录入三表读数 → 拍照加水印 → 本地 Room 暂存 → 检测网络 → Multipart 上传 → `DormManage.Api.exe`（Kestrel 自托管）业务处理 → 图片落盘 `D:\DormManage\Images\YYYYMM\住宿编号\` → 数据库写入关联 → `DormManage.Admin.exe` 通过 `UseStaticFiles` 直接暴露 `/uploads/*` 给浏览器在线预览 → 托盘程序 7×24 守护两个 EXE 进程并提供可视化配置入口。
...
**问题**：PDA 摄像头 800W~1300W 像素，单张照片 3~6MB，连续抄表 200 间住宿 = 800MB~1.2GB，会撑爆服务端带宽。
...
```powershell
cd "E:\AI工作目录\AI编程开发\JINGE开发\住宿管理系统\02-PDA-App"
...
param(
    ...
    [string]$LocalApk = "E:\AI工作目录\AI编程开发\JINGE开发\住宿管理系统\02-PDA-App\app\build\outputs\apk\release\pda-app-*.apk"
)
...
金智住宿管理系统 APP 图标出现在 PDA 桌面，点击即可启动。
...
sequenceDiagram
    ...
    M->>T: ⑲ StatusChanged(Running) 事件
    T->>T: ⑳ 触发 NotifyIconManager 联动：停止三色轮询（🩶⚪🟧→三色状态机）→ 托盘图标变黄/绿 + 菜单项联动
...
static void Main()
{
    ...
    trayIcon.ShowBalloonTip("金智住宿管理系统", "监控程序已启动\n双击图标打开设置", ...);
...
4. 任务栏右下角出现 🩶⚪🟧 三色轮询托盘图标（灰/白/橙，每 500ms 切换，启动阶段标识）

**日常使用（每天）**：
1. 用户登录 Windows → 托盘程序**自动启动**（注册表开机自启）
2. 托盘图标开始 🩶⚪🟧 三色轮询（启动阶段）
3. 自动拉起或右键托盘 → 「启动PDA服务」+「启动WEB服务」
4. 等待 30 秒（健康检查黄金窗口），等待期间图标保持轮询
5. 任一服务进入 Running 或 Crashed 后，停止轮询，切换到三色状态机：
   - 🟢 Green：双服务都 Running
   - 🟡 Yellow：仅一个 Running
   - 🔴 Red：双服务都异常
6. PDA 抄表员开始工作（扫码 → 上传到 :5100）
7. PC 管理员打开浏览器访问 `http://server:5001` 查看数据
...
| 顺序 | 阶段 | 执行者 | 关键动作 | 失败行为 |
|:---:|------|--------|---------|---------|
| **1** | **托盘程序启动** | `DormManage.TrayApp.exe` | 加载本地配置 → 通过 `AppConfigManager` 读取 `db_setting.json`（**密文 → AES-256 解密**）→ 创建 `NotifyIcon` 显示托盘图标 → 启动三色轮询动画（🩶灰/⚪白/🟧橙，每 500ms 切换）→ 启动右键菜单 | 托盘是常驻进程，本阶段不连接数据库；配置缺失时弹出「系统配置」窗口引导用户填写；轮询动画在双服务均 `Stopped` 时持续运行，直到任一服务进入 `Running`/`Crashed` 才停止 |
...
sequenceDiagram
    ...
    alt 连接成功
        DB-->>W: OK
        W-->>T: ⑩ 30s 黄金窗口健康检查通过
        T->>T: ⑪ 状态 → Running，触发事件 → 停止轮询 → 托盘图标变黄/绿
    else 连接失败
        DB-->>W: 失败 (TCP/Auth/库不存在)
        W-->>T: ⑫ 启动失败
        T->>W: ⑬ Process.Kill()
        T->>T: ⑭ 状态 → Crashed，触发事件 → 停止轮询 → 托盘图标变红
...
**核心关系**：
- 一间住宿（Dorm）→ 多次抄表记录（MeterRecord），按月去重
...
| `BedNumbers` 🆕 | NVARCHAR(500) | NULL | **床位号（v2.12.34 新增）：按住宿房间容量自动生成，从 1 起至 Capacity 上限为床位号；存储格式 `1,2,3,4`（逗号分隔）；当 Capacity 变更时自动重新生成** |
...
| `RecordId` | BIGINT | NOT NULL, FK→MeterRecord.RecordId (BIGINT) ON DELETE CASCADE | 关联抄表记录（v2.13.79 起 EF 模型对齐 long） |
...
- 主题色：Bootstrap 默认蓝（#0d6efd）+ 自定义品牌色（金智新材料 CI）
- 圆角：4px（按钮）/ 8px（卡片）
...
```
┌────────────────────────────────────────────────────────────────────────┐
│ 🏭 金智住宿管理系统     [首页] [记录] [统计] [用户] [配置]   admin 👤 退出│
├────────────────────────────────────────────────────────────────────────┤
│                                                                        │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐               │
│  │ 本月抄表   │  │ 已上传   │  │ 抄表员   │  │ 图片总数   │              │
...
- **分页**：每页 10 条，URL 携带 `?page=N&size=20`
...
```
┌────────────────────────────────────────────────────────────────────────┐
│ 🏭 金智抄表    抄表记录详情 #10245                          admin 👤 退出│
├────────────────────────────────────────────────────────────────────────┤
│ ┌─ 基本信息 ──────────────────────────────────────────────────────┐   │
│ │ 记录 ID：10245    抄表月份：2026-07    状态：✅ 正常              │   │
│ │ 住宿：D-301 (3号楼301室)    抄表员：张三 (PDA: PDA-SN-001)        │   │
...
| **状态可视化** | 启动阶段：托盘图标三色轮询（🩶灰/⚪白/🟧橙，每 500ms 切换），仅在双服务均 Stopped 时显示。运行阶段：红=双服务均异常，黄=单服务运行，绿=双服务都运行 |
...
| **状态可视化** | 启动阶段：托盘图标三色轮询（🩶灰/⚪白/🟧橙，每 500ms 切换）显示"启动中" | 红=双服务均异常，黄=单服务运行，绿=双服务都运行 |
| **危险操作隔离** | "退出"放最底部，与日常操作视觉分离 | 用 `ToolStripSeparator` 物理分组 |
| **二次确认** | "退出"会停止所有服务，必须弹窗确认 | `MessageBox.Show(..., YesNo, Question)` |
| **图标提示** | 启动阶段：托盘图标三色轮询（🩶灰/⚪白/🟧橙，每 500ms 切换）。运行阶段：托盘图标随状态变色（红=异常/黄=单服务/绿=双服务） | 使用 `System.Drawing` 动态生成实心圆点图标，启动轮询通过 `IconAnimator` 独立管理 |
...
**图标颜色状态机（v2.13.19 + v2.13.203 启动轮询动画规则）**：

| Api 状态 | Admin 状态 | 图标颜色 | 含义 |
|----------|-----------|---------|------|
| Stopped  | Stopped   | 🩶⚪🟧 轮询（灰/白/橙，每 500ms 切换） | **启动阶段**（图标轮询动画） |
| Running  | Running   | 🟢 Green | 双服务均正常 |
| Running  | 非 Running | 🟡 Yellow | 仅 Api 正常 |
| 非 Running | Running  | 🟡 Yellow | 仅 Admin 正常 |
| 非 Running | 非 Running | 🔴 Red | 双服务均异常 |

**启动轮询规则（v2.13.203）**：
- **启动条件**：双服务均处于 `Stopped` 状态
- **停止条件**：任一服务进入 `Running` 或 `Crashed` 状态
- **轮询参数**：颜色 Gray(#6c757d) → White(#f8f9fa) → Orange(#fd7e14) → 回到 Gray，每 500ms 切换
...
if (!createdNew) {
    MessageBox.Show("金智住宿管理系统托盘守护程序已在运行中。
...
系统采用基于 CDKEY 的软件授权机制，托盘端为注册校验唯一权威源，Admin/Api 通过 IPC 轮询获取状态。根据授权结果，系统进入三种互斥的运行模式：

| 运行模式 | 触发条件 | IsReadOnly() | IsTrialMode() | 写操作权限 | 参考文档 |
|:---:|:---|:---:|:---:|:---:|:---|
| **试用模式** | 未注册（RegInt=-1）且 UseTimes < 30 | false | true | ✅ 3 模块记录数受限 | [§233 三种运行模式定义](../233-三种运行模式定义-v2.13.180.md) |
| **注册模式** | 已注册且有效（RegInt=1，RegDate≥Today） | false | false | ✅ 全功能无限制 | [§233 三种运行模式定义](../233-三种运行模式定义-v2.13.180.md) |
| **只读模式** | 注册过期（RegDate<Today）或校验失败（RegInt=0） | true | false | ❌ 全部拦截 403 HTTP | [§233 三种运行模式定义](../233-三种运行模式定义-v2.13.180.md) |

> **完整规则**（包括试用模式的分模块记录上限、只读模式的白名单路径、Error 页面友好提示等）详见 [《三种运行模式定义》](../233-三种运行模式定义-v2.13.180.md)。
...
**现象**：两人几乎同时上传同一住宿（不可能但理论有），文件名相同被覆盖。
...
| 9 | 住宿档案导入 | 下载模板 → 填入 → 上传 | 成功导入，失败项有报告 |
...
| **v2.10** | **2026-07-09** | **新增「第 19 章 人员清单管理」+「第 20 章 费用管理」：人员清单含部门/员工类型/入职离职/工号/手机号/备注字段 + Excel 批量导入导出；费用管理含 BillingStandard 费用标准（热水/冷水/电单价 + 适用员工类型多选勾选）+ 按月统计 住宿费用清单 + 按月分摊 员工费用清单 + 双清单 Excel 导出；新增 4 张表（SysEmployee/BillingStandard/DormBilling/EmployeeBilling）+ 12+ API 端点 + 3 个 Web 后台页面 + 完整业务规则（在职分摊、适用类型筛选、平均分摊算法、发布控制、生效时间区间校验）** |
...
> **本方案（v2.13.19）系统性地将服务端架构由 IIS 反向代理重构为 .NET 8 自托管 EXE + 托盘管理程序守护，并配套三层进程防护、可视化配置、数据库配置双 UI 同步、人员清单管理、费用管理（费用标准+住宿账单+员工分摊）、日志审计、备份恢复、监控运维、安全合规等完整工程实践。所有部署、运维、升级均基于 EXE 进程模型，不再依赖 IIS。**
>
...
| `DormCode` | NVARCHAR(20) | NULL, FK→Dorm.DormCode | 关联住宿（NULL = 未分配） |
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  🏭 金智住宿管理系统  [首页] [记录] [统计] [人员] [住宿] [费用] [系统]  admin│
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
...
│                                                                          │
│  ┌────┬───────┬──────┬──────┬──────┬──────┬────────┬────────┬────┬────┬─────┐  │
│  │选择│工号   │姓名  │部门  │类型  │班组  │手机号  │入职日期 │住宿│状态│操作 │  │
│  ├────┼───────┼──────┼──────┼──────┼──────┼────────┼────────┼────┼────┼─────┤  │
...
> **本章定位**：建立基于抄表数据的费用管理体系。包含**费用标准配置**、**住宿月度费用统计**、**员工费用分摊计算**三大子模块，支持 Excel 报表导出。
...
| `ResidentCount` | INT | NOT NULL | 该住宿在住员工数（分摊用） |
...
| `DormBillingId` | INT | FK→DormBilling | 关联住宿费用记录 |
...
Response 200:
Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet
Content-Disposition: attachment; filename="住宿费用清单_2026-07.xlsx"
```

**Excel 列**：房号、地址、楼栋、计费月份、冷水用量(m³)、热水用量(m³)、电用量(度)、冷水费(元)、热水费(元)、电费(元)、总费用(元)、在住员工数、是否已发布

#### 20.4.2 Web UI：住宿费用清单页
...
- 总金额校验：Σ(分摊金额) = 住宿总费用（允许 ±0.01 元尾差）
...
**与 6.4 系统配置窗口的关联**：
- 托盘配置窗口增加「费用管理」Tab
- 显示当前生效标准、生成账单入口、查看住宿/员工账单

**与 5.4 Web 后台的关联**：
- 顶部导航增加 `[费用]` 菜单
- 子菜单：[费用标准] / [住宿费用清单] / [员工分摊费用]
...
| 5.4 Web 后台 | 新增 3 个页面（人员/住宿账单/员工账单） |
...
> - **费用管理**：3 级计算链（费用标准 → 住宿账单 → 员工分摊），含完整的业务规则、权限控制、报表导出
> - 与现有模块（数据库/PDA/托盘/Web后台/安全）全链路打通
> - 是方案 v2.10 的核心新增，也是系统从「工具」到「平台」的关键升级

---

## v2.13.111 备注：住宿档案列表「班组」列派生

**字段位置**：`DormDto.TeamNames`（v2.13.111 新增）

**数据源链路**：

| # | 实体 | 字段 | 说明 |
|---|------|------|------|
| 1 | `Dorm` | `DormCode` | 住宿房号（PK） |
| 2 | `DormBooking` | `DormCode` + `Status=2` | 当前在宿记录 |
| 3 | `SysEmployee` | `EmployeeId` + `TeamId` | 员工档案（真源） |
| 4 | `Team` | `Id` + `Name` + `SortOrder` | 班组字典 |

**派生 SQL 模式**：

```sql
-- v2.13.111 批量 JOIN 避免 N+1（IN 列表 + GroupBy + Dictionary）
SELECT b.DormCode, t.Name AS TeamName, t.SortOrder
FROM DormBooking b
INNER JOIN SysEmployee e ON b.EmployeeId = e.EmployeeId
INNER JOIN Team t ON e.TeamId = t.Id
WHERE b.DormCode IN (@CurrentPageDormCodes) AND b.Status = 2
GROUP BY b.DormCode, t.Id, t.Name, t.SortOrder
ORDER BY t.SortOrder, t.Id
```

**前端渲染规则**：

- 多个班组用 `bg-primary` Badge 拼接（按 SortOrder 升序 + 去重）
- **无人入住时 `<td>` 完全空白**，不渲染 `-` 或任何字符（用户追加要求）
- 与 v2.13.95 班次列 / v2.13.91 住宿详情班组列 / v2.13.97 Booking 列表班组列采用同 `EmployeeTeamMap` 模式
