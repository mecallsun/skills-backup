...
> 最近修订：2026-07-14
> 修订记录：
> - **v2.12.36（2026-07-14）— 系统启动顺序与运行期控制规范**：新增 §3.6.10，明确托盘先于服务启动、数据库连接失败即停止 Web/PDA 启动、数据库连接串以 Windows DPAPI 在本地加密存储（托盘启动时自动解密供 Web/PDA 使用）、托盘守护监测日志按 `YYYYMMDD_log` 命名写入本地日志目录、全局启动/停止/退出流程；与 §6、§8.1、§9.1 保持一致
...
> - **v2.11.7（2026-07-12）** — **托盘系统配置窗口职责收敛**：SettingsForm 仅保留核心服务端参数配置（PDA/Web 端口、数据库连接、图片存储路径、服务启停、保存参数），无角色权限控制；Web 端系统设置承载全部功能（服务管理、数据库、PDA版本、用户角色、备份恢复、系统集成、筛选缓存），受角色权限管控；双 UI 从"完整配置互补"调整为"托盘轻量运维 + Web 完整管理"
...
| 网络组件 | 要求 |
|---------|------|
| 服务器端口 | 5100（API）、5200（Web）、可选 5900（托盘远程） |
...
flowchart LR
    ...
    subgraph SERVER[Windows 服务器 EXE 进程]
        A8 -->|HTTP POST :5000| S1[DormManage.Api.exe]
    ...
    subgraph ADMIN[Web 后台 EXE 进程]
        W1[管理员浏览器 :8080] -->|HTTP GET| M1[DormManage.Admin.exe]
    ...
    subgraph DISK[服务器磁盘 D:\MeterImages]
...
**完整闭环**：PDA 扫码 → 匹配宿舍 → 录入三表读数 → 拍照加水印 → 本地 Room 暂存 → 检测网络 → Multipart 上传 → `DormManage.Api.exe`（Kestrel 自托管）业务处理 → 图片落盘 `D:\MeterImages\YYYYMM\宿舍编号\` → 数据库写入关联 → `DormManage.Admin.exe` 通过 `UseStaticFiles` 直接暴露 `/uploads/*` 给浏览器在线预览 → 托盘程序 7×24 守护两个 EXE 进程并提供可视化配置入口。
...
**示例**：
```
http://192.168.1.237:8080/downloads/pda-app-1.0.0-100-release-20260709.apk
http://192.168.1.100:8080/downloads/pda-app-latest.apk              ← 软链接（可选）
```
...
访问 `http://server:8080/downloads/` 会显示：
```
Index of /downloads
...
✓ 下载地址：http://192.168.1.237:8080/downloads/pda-app-1.0.0-100-release-20260709.apk
```
...
1. PC 浏览器输入：`http://192.168.1.237:8080/downloads/`
...
private void DoDownload()
{
    ...
    var url = _configService.GetPdaAppDownloadUrl();     // http://server:8080/downloads/
...
│                                             │
│ 访问地址：                                    │
│ http://192.168.1.237:8080/downloads/         │
│                                             │
│ 文件列表：                                    │
│ • pda-app-1.0.0-100-release-20260709.apk     │
│   (12.5 MB)                                 │
...
- 在地址栏输入：`http://192.168.1.237:8080/downloads/`
- 或直接输入完整 APK URL：`http://192.168.1.237:8080/downloads/pda-app-1.0.0-100-release-20260709.apk`
...
金戈水电抄表 APP 图标出现在 PDA 桌面，点击即可启动。
...
Response: {
    "versionCode": 101,
    "versionName": "1.0.1",
    "downloadUrl": "http://server:8080/downloads/pda-app-1.0.1-101-release-20260720.apk",
...
在 Web 后台生成 QR Code：
```
http://192.168.1.237:8080/downloads/pda-app-latest.apk
```
...
| **下载 URL（局域网）** | `http://192.168.1.237:8080/downloads/*.apk` | 浏览器 |
| **下载 URL（本地）** | `http://127.0.0.1:8080/downloads/*.apk` | 本机测试 |
| **下载 URL（HTTPS）** | `https://server:8443/downloads/*.apk` | 跨网段 |
| **目录浏览 URL** | `http://server:8080/downloads/` | 列出所有 APK |
...
用户输入 → 点击 [登录]
  ↓
1. 前端校验（用户名/密码/设备号非空）
2. 读取服务器 URL（默认 http://192.168.1.237:5000）
...
├──────────────────────────────┤
│ ┌─ 服务器连接 ──────────────┐│
│ │ 服务器 URL：http://192.168.1.237:5000│
│ │ [测试连接] [修改]            ││
│ └────────────────────────────┘│
│                              │
│ ┌─ 抄表设置 ──────────────────┐│
...
|--------|---------|---------|------|---------|
| **API 服务** | `DormManage.Api.exe` | 5000（可配） | 接收 PDA 抄表数据上传、数据库写入 | 由托盘程序拉起 |
| **Admin 服务** | `DormManage.Admin.exe` | 8080（可配） | 提供后台管理 Web UI、静态文件托管 | 由托盘程序拉起 |
| **Tray 守护** | `DormManage.TrayApp.exe` | — | 7×24 守护前两个 EXE；提供托盘菜单 + 配置 UI | 开机自启 |
...
var apiPort = builder.Configuration.GetValue<int>("Kestrel:Endpoints:Http:Url", 5000);
...
Response 成功:
  {
    ...
    "data": {
      ...
      "dormCode": "D-301",
      "imageUrls": [
        "http://server:8080/uploads/202607/D-301/20260708_103055_cold.jpg",
        "http://server:8080/uploads/202607/D-301/20260708_103055_hot.jpg",
        "http://server:8080/uploads/202607/D-301/20260708_103055_electric.jpg"
      ],
      "duplicate": false
    }
  }
...
Kestrel 静态托管（Admin EXE 中配置）：
  UseStaticFiles 映射 /uploads → D:\MeterImages
...
```csharp
// Admin/Program.cs
var imageRoot = builder.Configuration["ImageBasePath"] ?? @"D:\MeterImages";
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=192.168.1.237,1433;Database=DormManageDB;User Id=__DB_USER__;Password=__DB_PASSWORD__;TrustServerCertificate=True;MultipleActiveResultSets=True;Connection Timeout=30"
  },
  "Kestrel": {
    "Endpoints": {
...
│   │       ├── css/
│   │       ├── js/
│   │       ├── uploads/                ← Kestrel 直接托管的图片目录（链接到 D:\MeterImages）
│   │       └── downloads/              ← APK 下载目录
│   └── TrayApp/
│       └── DormManage.TrayApp.exe      ← ② 控制中枢
│
├── D:\MeterImages\                              ← 数据盘（独立盘符，可选）
│   ├── 202607\D-301\
│   │   ├── 20260708_103055_cold.jpg
│   │   ├── 20260708_103055_hot.jpg
│   │   └── 20260708_103055_electric.jpg
...
sequenceDiagram
    ...
    A->>A: ㉙ JWT 鉴权 + 业务处理 + 图片落盘 D:\MeterImages
...
static void Main()
{
    ...
    trayIcon.ShowBalloonTip("金戈水电抄表", "监控程序已启动\n双击图标打开设置", ...);
...
| 客户端 | 协议 | 目标进程 | 端点 |
|--------|------|---------|------|
| **PDA 终端** | HTTP | `DormManage.Api.exe`（:5000） | `POST /api/auth/login`<br/>`POST /api/meter/upload`<br/>`GET /api/meter/records` |
| **PC 浏览器** | HTTP | `DormManage.Admin.exe`（:8080） | `GET /` 后台首页<br/>`GET /Account/Login`<br/>`GET /Records` 数据列表<br/>`GET /uploads/*` 图片预览 |
| **PDA ↔ 图片** | HTTP | `DormManage.Admin.exe`（:8080） | `GET /uploads/202607/D-301/xxx.jpg`（Admin 通过 Kestrel `UseStaticFiles` 托管 D:\MeterImages） |

**关键设计**：
- **PDA 上传图片**只调用 Api.exe（:5000），Api 负责写盘到 `D:\MeterImages\`。
- **浏览器查看图片**只调用 Admin.exe（:8080），Admin 通过 `UseStaticFiles(FileProvider=PhysicalFileProvider("D:\MeterImages"))` 直接提供 HTTP 文件服务。
- 两个 EXE **共享同一数据盘** `D:\MeterImages\`，因此上传后立即可被浏览器预览。
...
3. 等待 30 秒（健康检查黄金窗口）
4. 托盘图标变绿 → 服务就绪
5. PDA 抄表员开始工作（扫码 → 上传到 :5000）
6. PC 管理员打开浏览器访问 `http://server:8080` 查看数据
...
| 顺序 | 阶段 | 执行者 | 关键动作 | 失败行为 |
|:---:|------|--------|---------|---------|
| **1** | **托盘程序启动** | `DormManage.TrayApp.exe` | 加载本地配置 → 解析 `appsettings.json` 中的 `ConnectionStrings:Default`（**密文 → DPAPI 解密**）→ 创建 `NotifyIcon` 显示托盘图标 → 启动右键菜单 | 托盘是常驻进程，本阶段不连接数据库；配置缺失时弹出「系统配置」窗口引导用户填写 |
| **2** | **Web 服务启动（依赖 DB）** | 托盘拉起 `DormManage.Admin.exe` | 进程启动 → 读取 `appsettings.json` → **先尝试连接数据库**（启动期探测，< 5s） | **数据库连接失败 ⇒ 立即停止 Web 启动**（Kill 刚拉起的进程，标记为 `Error`，托盘图标变红 + 弹窗「数据库连接失败，请检查配置」） |
| **3** | **PDA 服务启动（依赖 DB）** | 托盘拉起 `DormManage.Api.exe` | 进程启动 → 读取 `appsettings.json` → **先尝试连接数据库**（同上） | **数据库连接失败 ⇒ 立即停止 PDA 启动**（Kill 刚拉起的进程，标记为 `Error`，托盘图标变红 + 弹窗） |
| **4** | **数据库参数在线修改（按需）** | 用户 → 托盘菜单 → 「系统配置」 | 修改服务器/端口/库名/账号/密码 → 点击「测试连接」→ 通过后点击「保存」→ **新连接串经 DPAPI 加密** 写回 `%LOCALAPPDATA%\DormManage\config\{Api\|Admin}\appsettings.json` | 保存前若未通过测试，禁用「保存」按钮 |
| **5** | **系统重连数据库** | 托盘 `ProcessManager` | 检测到配置变更 → 按需重启 Web/PDA → 重启时使用新连接串连接数据库 | 仍然失败则保持 `Error` 状态，等待下一次配置修改 |
...
sequenceDiagram
    ...
    participant T as TrayApp.exe
    participant FS as 文件系统
    participant DP as Windows DPAPI
    ...
    U->>T: ① 启动托盘（开机自启 / 手动）
    T->>FS: ② 读取 %LOCALAPPDATA%\DormManage\config\appsettings.json
    FS-->>T: 返回密文配置（连接串已加密）
    T->>DP: ③ Decrypt(ConnectionStrings.Default)
    DP-->>T: 返回明文连接串（内存中）
    ...
    Note over U,T: 阶段 4 - 用户修改数据库参数（按需）
    U->>T: ㉑ 右键托盘 → 系统配置
    T->>U: ㉒ 弹出 SettingsForm
    U->>T: ㉓ 修改数据库连接字段
    U->>T: ㉔ 点击「测试连接」→ 8 步深度验证
    T-->>U: ㉕ 测试通过 / 失败明细
    U->>T: ㉖ 点击「保存」→ 写入前加密
    T->>DP: ㉗ Protect(明文连接串, DataProtectionScope.CurrentUser)
    DP-->>T: 返回密文
    T->>FS: ㉘ 原子写入 appsettings.json（密文）
    T-->>U: ㉙ 提示「配置已保存，是否重启服务？」

    Note over T,DB: 阶段 5 - 系统重连数据库
    U->>T: ㉚ 点击「是，重启服务」
    T->>W: ㉛ StopService("Admin") + StartService("Admin")
    T->>P: ㉜ StopService("Api") + StartService("Api")
    W->>DB: ㉝ 使用新连接串连接
    P->>DB: ㉞ 使用新连接串连接
    DB-->>W: OK
    DB-->>P: OK
...
##### 3.6.10.3 数据库连接串本地加密存储（DPAPI 方案）

> **技术选型说明**：连接串包含数据库账号密码，必须以**可逆加密**方式保护（用于运行时解密连接）。**MD5 / SHA-256 均为单向哈希，无法解密**，故不适用。本系统采用 **Windows DPAPI（Data Protection API）**：
> - ✅ **可逆**：`ProtectedData.Unprotect()` 可在同用户、同机器解密
> - ✅ **零密钥管理**：密钥由 Windows 操作系统根据当前登录用户 + 机器自动派生
> - ✅ **强权限隔离**：其他 Windows 用户无法解密
> - ✅ **免配置**：无需 .pfx / .key 文件、无需密码短语
> - ✅ **与现有体系一致**：§4.8.2 敏感字段加密、§11.2 JWT Secret 同样使用 DPAPI 加密存储

**加密流程**（保存时）：

```csharp
// 06-TrayApp/Services/ConfigService.cs
using System.Security.Cryptography;

public void SaveConnectionString(string connStr)
{
    // 1. 明文 → DPAPI 加密（绑定到当前 Windows 用户）
    var plainBytes = Encoding.UTF8.GetBytes(connStr);
    var protectedBytes = ProtectedData.Protect(
        plainBytes,
        optionalEntropy: null,
        scope: DataProtectionScope.CurrentUser);  // 仅当前用户可解

    // 2. 密文 → Base64 → 写入 JSON
    var config = LoadConfig();
    config.ConnectionStrings.Default = Convert.ToBase64String(protectedBytes);
    AtomicWriteJson(configPath, config);  // 原子写入 + .bak 备份
}
```
...
{
    var config = LoadConfig();
    var protectedBytes = Convert.FromBase64String(config.ConnectionStrings.Default);
    ...
    {
        // 密文损坏 / 用户变更 / 机器变更 → 弹出系统配置窗口
        _trayIcon.ShowSettingsForm("本地配置已加密且无法解密，请重新配置数据库参数");
        throw;
    }
}
```
...
| 阶段 | 传递方式 | 落地位置 |
|------|---------|---------|
| 托盘 → Web/PDA | **环境变量** `DormManage_DB_CONN`（明文，进程内） | `ProcessStartInfo.Environment["DormManage_DB_CONN"]` |
| Web/PDA 内部 | `Program.cs` 读环境变量 → 覆盖 `appsettings.json` 的 `ConnectionStrings:Default` | 内存中替换，不回写磁盘 |
| 持久化 | `%LOCALAPPDATA%\DormManage\config\*.json` 中始终存**密文** | DPAPI `Protect` 后的 Base64 |

**为什么不直接明文保存**：
- 数据库账号密码明文存于磁盘 ⇒ 服务器被入侵即可直接登录数据库
- 备份文件 / 镜像文件泄漏 ⇒ 数据库密码一同泄漏
- DPAPI 加密后，即使整个 `%LOCALAPPDATA%` 目录被复制到其他机器或用户，**依然无法解密**

**为什么不用 AES-256 自行加密**：
- 需要单独管理 AES 密钥（Key）— Key 存哪里？存文件则失去意义，存 DPAPI 则多层嵌套
- DPAPI 已经过 Windows 系统级加固，使用 AES 自定义方案需要自行实现 PBKDF2 + Salt + Key 派生 + 完整性校验
- DPAPI 在所有 Windows 版本（Win10 / Win11 / Server 2016+）原生支持，零额外依赖
...
| Web/PDA 启动成功 | `START` | `[START] Admin 启动成功 PID=12345 端口=8080` |
...
| DPAPI 加密 | §4.8.2 敏感字段加密、§11.2 JWT Secret | 复用：与密码字段、JWT 密钥使用相同加密体系 |
...
```
DormManage.Api.exe    ← 独立进程，监听 :5000
DormManage.Admin.exe  ← 独立进程，监听 :8080
...
| **B. 单 EXE + 双端口** | `Service.exe`（同时监听 :5000 + :8080） + `TrayApp.exe` | 2 | ❌ 共享进程 | ✅ 低（2 个 EXE） |
| **C. 单 EXE + Role 参数** | `DormManage.exe --role=api` 和 `--role=admin` + `TrayApp.exe` | 3 | ✅ 隔离 | ✅ 低（1 个 EXE + 配置） |
...
- `TrayIconManager.cs`：托盘菜单「启动PDA服务」「启动WEB服务」双入口
...
**架构**：
```
DormManage.Service.exe --role=api    → 启动实例 1（监听 :5000）
DormManage.Service.exe --role=admin  → 启动实例 2（监听 :8080）
DormManage.TrayApp.exe               → 控制中枢（守护前两者）
```
...
var apiPsi = new ProcessStartInfo {
    ...
    Arguments = "--role=api --urls \"http://localhost:5000\"",
    // ...
};
var adminPsi = new ProcessStartInfo {
    FileName = "DormManage.Service.exe",
    Arguments = "--role=admin --urls \"http://localhost:8080\"",
    // ...
};
```
...
**形态**：`Service.exe` 启动后同时监听 :5000 + :8080。
...
| `AbsolutePath` | NVARCHAR(500) | NOT NULL | 绝对路径（如 D:\MeterImages\202607\D-301\...） |
...
| 配置 Key | 默认值 | 说明 |
|---------|--------|------|
| `image_base_path` | `D:\MeterImages` | 图片存储根目录 |
...
│ │ HR   │[人力资源..│ http://hr..:8080 │ hr_user  │ ●●●●●  │☑️  │🔌  │连接成功│ │
...
├── 01_DDL_Schema.sql              ← 首次部署（__DB_USER__ 执行）
├── 02_Seed_Data.sql               ← 首次部署（__DB_USER__ 执行）
├── Migrations/                    ← 增量迁移
...
| 手机号 | 对称加密（AES-256） | Windows DPAPI |
| 身份证号 | 对称加密（AES-256） | Windows DPAPI |
| JWT Secret | 配置文件 + Windows DPAPI 加密存储 | 配置中心 |
...
-- 创建登录账号
CREATE LOGIN [__DB_USER__] WITH PASSWORD = N'__DB_PASSWORD__',
...
-- 授权（最小权限原则）
ALTER ROLE [db_owner] ADD MEMBER [__DB_USER__];  -- 开发环境
-- 生产环境建议细分权限：db_datareader + db_datawriter + 执行存储过程权限
GO
```

`01_DDL_Schema.sql`（__DB_USER__ 执行）：详见 `01-Database/01_DDL_Schema.sql`，包含全部 9 张表 + 索引 + 约束。

`02_Seed_Data.sql`（__DB_USER__ 执行）：
...
- 独立 Kestrel 实例监听 8080
- 独立应用域，故障不影响 API
- 独立配置文件，端口可独立修改
- 静态文件由该进程托管，浏览器访问 `http://server:8080/uploads/*` 读取图片
...
```
┌────────────────────────────────────────────────────────────────────────┐
│ 🏭 金戈水电抄表     [首页] [记录] [统计] [用户] [配置]   admin 👤 退出│
├────────────────────────────────────────────────────────────────────────┤
│                                                                        │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐               │
│  │ 本月抄表   │  │ 已上传   │  │ 抄表员   │  │ 图片总数   │              │
...
│                                                                        │
│ ┌──────────────────────────────────────────────────────────────────┐ │
│ │ID |用户名  |角色 |真实姓名 |电话        |状态 |最后登录  |操作   │ │
│ ├──────────────────────────────────────────────────────────────────┤ │
│ │1  |admin   |管理员|系统管理员|138****0001|✅ 启用|2026-07-09|编辑 禁用│ │
│ │2  |pda001  |PDA  |张三     |138****0002|✅ 启用|2026-07-09|编辑 禁用│ │
│ │3  |pda002  |PDA  |李四     |138****0003|✅ 启用|2026-07-08|编辑 禁用│ │
│ │4  |view01  |只读|王五     |138****0004|❌ 禁用|2026-07-05|编辑 启用│ │
│ └──────────────────────────────────────────────────────────────────┘ │
│                                                                        │
│                  [◀ 上一页] 1 2 [下一页 ▶]                            │
└────────────────────────────────────────────────────────────────────────┘
...
│                                                                        │
│ ┌─ 数据库连接 ──────────────────────────────────────────────────────┐ │
│ │ 服务器：[192.168.1.237___]  端口：[1433]                          │ │
│ │ 数据库：[DormManageDB______]  账号：[__DB_USER_____]  密码：[●●●●●●]│ │
│ │                                            [🔌 测试连接]           │ │
│ └───────────────────────────────────────────────────────────────────┘ │
│                                                                        │
│ ┌─ 服务端口 ────────────────────────────────────────────────────────┐ │
│ │ API 端口：[5000]   Admin 端口：[8080]                            │ │
│ └───────────────────────────────────────────────────────────────────┘ │
│                                                                        │
│ ┌─ 图片存储 ────────────────────────────────────────────────────────┐ │
│ │ 路径：[D:\MeterImages_______________________________] [📁 浏览]   │ │
│ └───────────────────────────────────────────────────────────────────┘ │
│                                                                        │
│ ┌─ 业务规则 ────────────────────────────────────────────────────────┐ │
...
| **紧凑型图标导航条 (Tier 2)** | 一级菜单（仅图标 + Tooltip，类别色一致） | `_Layout.cshtml` |
| **Tab 页签栏 (Tier 3)** | 已打开页面切换 + 关闭 + 固定 + 拖拽 + localStorage 持久化 | `_Layout.cshtml` + `~/js/tab-manager.js` |
| **共用页头内容区 (Tier 4)** | 图标 + 标题 + 总数 + 操作按钮（btn-group） | `_Layout.cshtml` 内 `@RenderBody()` 顶部 |
...
##### 5.4.11.1 导航升级说明（v2.12.0 新增）

> ⚠️ **强制**：自 v2.12.0 起，所有 Web 后台页面 **必须** 基于 `_Layout.cshtml` 实现四层架构，**禁止** 任何页面独立绘制导航或页头。

| # | 旧模式（v2.11） | 新模式（v2.12.0） |
|---|----------------|------------------|
| 1 | 单层顶部导航（Logo + 主菜单横向排列） | **顶部品牌栏**（48px）+ **图标导航条**（56px）+ **Tab 页签栏**（40px） |
| 2 | 点击菜单 → 整页 URL 跳转 | 点击图标 → **新增 Tab**，原 Tab 保留 |
| 3 | 浏览器 history 管理后退 | localStorage 持久化 Tab 列表，支持任意关闭 |
| 4 | 仅桌面端适配 | 桌面/平板/移动三端响应式（移动端改底部 Tab Bar） |
| 5 | 列表页与导航各页独立 | **所有页面共用页头 + Tab 框架**，仅 Tier 4 内容独立 |
...
| **健康检查** | 每 10 秒 HTTP 探测 `/health` 和 `/api/dbinit/ping` |
| **可视化配置** | GUI 编辑数据库连接、端口、图片路径、服务器域名 |
| **日志审计** | 实时显示 Guardian 日志，记录非授权进程 Kill 事件 |
| **开机自启** | 写注册表 `HKCU\...\Run`，无需管理员权限 |
| **数据库连通性测试** | GUI 内一键 TCP 端口 + SQL 登录测试 |
| **守护扫描器** | 每 2 秒扫描系统所有 `DormManage.*.exe` 进程，清理未授权或孤儿进程（详见第 7 章） |
...
│ ├─ StartService(name) / StopService(name)    │
│ ├─ StartHealthCheckTimer (10s)              │
│ ├─ StartGuardianTimer (2s)                   │
│ └─ 事件：StatusChanged → TrayIcon + Settings │
├─────────────────────────────────────────────┤
│ ConfigService (配置持久化)                   │
│ ├─ LoadConfig() / SaveConfig()              │
│ ├─ LOCALAPPDATA ↔ publish 双向迁移            │
│ └─ 原子写入（.tmp → Move 覆盖）              │
├─────────────────────────────────────────────┤
│ GuardianLog (审计日志)                       │
│ └─ %LOCALAPPDATA%\DormManage\guardian\      │
│    guardian-YYYYMMDD.log (30 天滚动)        │
└─────────────────────────────────────────────┘
```
...
```
┌──────────────────────────────────┐
│        任务栏监控程序菜单         │  ← 菜单标题（可选，OS 自动绘制）
├──────────────────────────────────┤
│  启动PDA服务                     │  ← 第 1 组（PDA 服务控制）
│  停止PDA服务                     │
├──────────────────────────────────┤
│  启动WEB服务                     │  ← 第 2 组（WEB 服务控制）
│  停止WEB服务                     │
├──────────────────────────────────┤
│  系统配置                        │  ← 第 3 组（独立操作）
├──────────────────────────────────┤
...
| # | 菜单项 | 分组 | 默认状态 | 启用条件 | 点击行为 |
|---|--------|------|---------|---------|---------|
| 1 | **启动PDA服务** | PDA 服务控制 | 可用 | PDA 处于 `Stopped` / `Error` | 调用 `ProcessManager.StartService("Api")` |
| 2 | **停止PDA服务** | PDA 服务控制 | 禁用 | PDA 处于 `Running` / `Starting` | 调用 `ProcessManager.StopService("Api")` |
| 3 | **启动WEB服务** | WEB 服务控制 | 可用 | WEB 处于 `Stopped` / `Error` | 调用 `ProcessManager.StartService("Admin")` |
| 4 | **停止WEB服务** | WEB 服务控制 | 禁用 | WEB 处于 `Running` / `Starting` | 调用 `ProcessManager.StopService("Admin")` |
| 5 | **系统配置** | 独立操作 | 始终可用 | — | 弹出 `SettingsForm`（4 Tab 设置窗） |
| 6 | **退出系统** | 危险操作 | 始终可用 | — | 二次确认后停止所有服务并退出托盘 |

**菜单分组规则**：用 `ToolStripSeparator` 在 ①「PDA 启动/停止」、②「WEB 启动/停止」、③「系统配置」、④「退出系统」之间分隔，形成 4 个视觉分组。

#### 6.3.3 设计原则

| 原则 | 说明 | 实现细节 |
|------|------|---------|
| **简洁至上** | 菜单项总数 ≤ 6，避免用户在紧急情况下找不到操作 | 不放"查看守护日志"等次要项；如确需可在「系统配置」内 Tab 提供 |
| **服务-操作对应** | 启动 / 停止成对出现，便于一眼看到当前能做什么 | `Enabled` 属性随 `ServiceStatus` 动态切换 |
| **状态联动** | 服务运行中时禁用"启动"、启用"停止"，反之亦然 | 订阅 `ProcessManager.StatusChanged` 事件实时刷新 |
| **危险操作隔离** | "退出系统"放最底部，与日常操作视觉分离 | 用 `ToolStripSeparator` 物理分组 + 字体加粗 |
| **二次确认** | "退出系统"会停止所有服务，必须弹窗确认 | `MessageBox.Show(..., YesNo, Question)` |
| **图标提示** | 托盘图标随状态变色（灰=停止、绿=运行、橙=启动中、红=异常） | 见 6.3.5 节实现要点 |
...
```csharp
// TrayIconManager.BuildContextMenu() —— 与最终实现对齐
private void BuildContextMenu()
{
    _contextMenu = new ContextMenuStrip();

    // === 第 1 组：PDA 服务 ===
    _startApiItem = new ToolStripMenuItem("启动PDA服务");
    _startApiItem.Click += (s, e) => { _processManager.StartService("Api"); UpdateMenuState(); };
    _stopApiItem  = new ToolStripMenuItem("停止PDA服务");
    _stopApiItem.Click  += (s, e) => { _processManager.StopService("Api");  UpdateMenuState(); };
    var sep1 = new ToolStripSeparator();

    // === 第 2 组：WEB 服务 ===
    _startAdminItem = new ToolStripMenuItem("启动WEB服务");
    _startAdminItem.Click += (s, e) => { _processManager.StartService("Admin"); UpdateMenuState(); };
    _stopAdminItem  = new ToolStripMenuItem("停止WEB服务");
    _stopAdminItem.Click  += (s, e) => { _processManager.StopService("Admin");  UpdateMenuState(); };
    var sep2 = new ToolStripSeparator();

    // === 第 3 组：系统配置 ===
    var settingsItem = new ToolStripMenuItem("系统配置");
    settingsItem.Click += (s, e) => ShowSettingsOnUiThread();

    // === 第 4 组：退出系统 ===
    var exitItem = new ToolStripMenuItem("退出系统");
    exitItem.Click += (s, e) => QuitOnUiThread();

    _contextMenu.Items.AddRange(new ToolStripItem[] {
        _startApiItem, _stopApiItem, sep1,
        _startAdminItem, _stopAdminItem, sep2,
...
private void UpdateMenuState()
{
    ...
    if (services.TryGetValue("Api", out var api))
    {
        _startApiItem!.Enabled  = api.Status is ServiceStatus.Stopped or ServiceStatus.Error;
        _stopApiItem!.Enabled   = api.Status is ServiceStatus.Running or ServiceStatus.Starting;
    }

    if (services.TryGetValue("Admin", out var admin))
    {
        _startAdminItem!.Enabled = admin.Status is ServiceStatus.Stopped or ServiceStatus.Error;
        _stopAdminItem!.Enabled  = admin.Status is ServiceStatus.Running or ServiceStatus.Starting;
    }

    // 同步刷新托盘图标颜色与 ToolTip 文字
    _notifyIcon.Icon = CreateStatusIcon(_processManager.GetCombinedStatus());
    _notifyIcon.Text = $"金戈水电抄表\nPDA: {StatusText(api.Status)}\nWEB: {StatusText(admin.Status)}";
}

// 订阅服务状态变化事件 → 实时刷新菜单
_processManager.StatusChanged += _ => {
    if (_contextMenu?.InvokeRequired == true)
        _contextMenu.Invoke(new Action(UpdateMenuState));
    else
        UpdateMenuState();
};
```

**与最终代码一致性**：上述代码片段已与 `06-TrayApp/TrayIconManager.cs` 的 `BuildContextMenu()` / `UpdateMenuState()` 严格对齐，开发与运维团队可直接以此为参照比对实现。
...
> **v2.11.7 重要变更**：系统配置窗口职责收敛为仅配置核心服务端参数（PDA/Web 端口、数据库连接、图片存储路径、服务启停、保存参数），**无角色权限项控制**。其他所有功能（PDA 版本管理、用户角色、备份恢复、系统集成、筛选缓存等）均在 Web 端实现，受角色权限管控。

#### 6.4.1 窗口样式图示

点击托盘菜单「系统配置」后弹出的 `SettingsForm` 窗口（参考样式 — 单页紧凑布局，仅核心服务端参数）：
...
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│   PDA服务端口号：[  5000  ]   [ 启动 ]  [ 停止 ]  运行中           │
│   Web服务端口号：[  8080  ]   [ 启动 ]  [ 停止 ]  运行中           │
│                                                                    │
│   图片保存路径：[ D:\MeterImages____________________________________ ]│
│                                                                    │
│   数据库  服务器[192.168.1.237]  端口号[1433]  账号[__DB_USER__]  密码[●●●●●●●●●●]│
│                                                                    │
│   服务器域名 ：[ 127.0.0.1_________________________________________ ]│
│                                                                    │
│                       [ 保存 ]      [ 关闭 ]      [ 退出系统 ]    │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
...
**窗口规格**：
- 标题：`系统配置`
- 尺寸：宽 720 × 高 380（紧凑布局，单页全部可见，无需滚动）
- 字体：Microsoft YaHei UI 9pt
- 关闭方式：标题栏右上角 `[X]` 按钮 + 底部 `关闭` 按钮等价
...
| # | 字段 | 类型 | 默认值 | 必填 | 校验规则 |
|---|------|------|--------|------|---------|
| 1 | **PDA 服务端口号** | NumericUpDown | 5000 | 是 | 1–65535，建议 1024–65535 避免与系统服务冲突 |
| 2 | **Web 服务端口号** | NumericUpDown | 8080 | 是 | 同上 |
| 3 | **PDA 服务启动** | Button | — | — | 实时联动 `ServiceStatus`（详见 6.4.4） |
| 4 | **PDA 服务停止** | Button | — | — | 同上 |
| 5 | **Web 服务启动** | Button | — | — | 实时联动 `ServiceStatus`（详见 6.4.4） |
| 6 | **Web 服务停止** | Button | — | — | 同上 |
| 7 | **运行状态文字** | Label | `运行中` / `已停止` / `启动中...` / `异常` | — | 仅显示，不可编辑 |
| 8 | **图片保存路径** | TextBox | `D:\MeterImages` | 是 | 必须为绝对路径且目录可写 |
| 9 | **数据库服务器** | TextBox | `192.168.1.237` | 是 | IP / 主机名 / `host,port` / `host\instance` |
| 10 | **数据库端口号** | TextBox | `1433` | 是 | 1–65535 |
| 11 | **数据库账号** | TextBox | `__DB_USER__` | 是 | 非空 |
| 12 | **数据库密码** | TextBox (PasswordChar) | — | 是 | 非空，使用 `UseSystemPasswordChar=true` 掩码 |
| 13 | **服务器域名** | TextBox | `127.0.0.1` | 是 | IP / 域名 / `localhost`，PDA 用此访问 |
| 14 | **保存** | Button | — | — | 收集所有字段 → 校验 → 写盘 → 提示重启 |
| 15 | **关闭** | Button | — | — | 放弃修改，关闭窗体 |
| 16 | **退出系统** | Button | — | — | 保存并退出托盘（等同于托盘菜单「退出系统」） |

> **v2.11.7 变更**：移除了 `APP下载` 和 `打开网址` 按钮，这两个功能已迁移至 Web 端系统设置模块。

#### 6.4.3 与当前实现的差异（v2.0 → v2.1 → v2.11.7 演进）

v2.0 的 `06-TrayApp/SettingsForm.cs` 实现采用 **4 Tab 布局**，与本参考样式的单页布局存在差异，演进路径如下：

| 维度 | 当前 v2.0 实现（4 Tab） | 参考样式 / v2.1 目标（单页） | 演进动作 |
|------|------------------------|-----------------------------|---------|
| **布局** | TabControl 4 个 Tab（服务管理 / 数据库 / 存储 / 系统） | **单页紧凑布局**（所有字段一屏可见） | 移除 TabControl，重排控件到单页 |
| **窗口尺寸** | 680 × 530 | **720 × 380**（更紧凑） | 调整 Form Width/Height |
| **开机自启** | 含 CheckBox「开机自动启动监控程序」 | **不含**（与图片一致） | 移除 `_chkAutoStart`，如需保留功能改为「系统 Tab」说明文字 |
| **测试连接按钮** | 含「🔌 测试连接」按钮（TCP+SQL 验证） | **不含**（与图片一致） | 移除按钮，`DbTestHelper` 内部功能保留为内部测试方法 |
| **服务启动/停止按钮位置** | 服务 Tab 中 | **主页顶部 PDA / Web 行**（与端口号同行） | 调整 `ServiceRow()` 布局 |
| **状态文字位置** | 服务 Tab 端口号右侧 | **主页顶部启动按钮右侧** | 同上 |
| **底部按钮** | 测试连接 + 退出系统 + 关闭 + 保存（4 个） | **保存 + 关闭 + 退出系统**（3 个，无测试连接） | 移除测试连接按钮 |
| **数据库字段布局** | 两行：服务器+端口 / 账号+密码 | **单行**：服务器+端口+账号+密码 | 重排为单行 4 列 |
| **服务器域名** | 系统 Tab 内 | **主页底部** | 调整位置 |

> **v2.11.7 额外变更**：移除 `APP下载` 和 `打开网址` 按钮，功能已迁移至 Web 端系统设置模块。

**v2.1 落地步骤**：
1. 重构 `SettingsForm.cs`，移除 TabControl
2. 按参考样式布局控件（建议用 TableLayoutPanel 实现响应式对齐）
3. 保留 `DbTestHelper` 类（供未来"高级配置"对话框使用）
4. 保留 `AutoStartManager`（可通过托盘菜单新增「开机自启」独立项）

**v2.11.7 落地步骤**：
1. 移除 `APP下载` 按钮及相关代码
2. 移除 `打开网址` 按钮及相关代码
3. 将 PDA 服务启动/停止按钮拆分为独立的启动和停止两个按钮
4. 确认系统配置窗口无角色权限项控制，移除任何权限判断代码

#### 6.4.4 服务状态联动逻辑

启动/停止按钮的 `Enabled` 随 `ServiceStatus` 实时联动：

| 状态 | 启动按钮 | 停止按钮 | 状态文字 |
|------|---------|---------|---------|
| Stopped | ✅ Enabled | ❌ Disabled | `已停止` |
| Starting | ❌ Disabled | ✅ Enabled | `启动中...` |
| Running | ❌ Disabled | ✅ Enabled | `运行中` |
| Stopping | ❌ Disabled | ❌ Disabled | `停止中...` |
| Error | ✅ Enabled | ❌ Disabled | `异常` |

实现订阅 `ProcessManager.StatusChanged` 事件 → 跨线程 `Invoke` → 刷新按钮与文字。
...
用户点击 [保存]
    ↓
1. 收集所有字段到 TrayAppConfig 对象
    ↓
2. 前端校验
   ├─ 数据库服务器为空？→ Alert("服务器地址不能为空") return
   ├─ 数据库账号为空？  → Alert("数据库账号不能为空") return
   ├─ 图片保存路径为空？→ Alert("图片存储路径不能为空") return
   └─ 通过 → 继续
    ↓
3. 应用端口到 Api / Admin 的 appsettings.json（Kestrel Url）
    ↓
4. 调用 ConfigService.SaveConfig(cfg)
   ├─ 原子写入 LOCALAPPDATA（主路径）
   ├─ 原子写入 publish 路径（兼容回退）
   └─ 写入 %LOCALAPPDATA%\DormManage\settings.json（本地偏好）
    ↓
5. 保存成功 → 弹窗确认重启
   "配置已保存！是否重启服务以应用新配置？"
   ├─ 是 → ProcessManager.RestartAll()
   └─ 否 → 仅关闭窗体
```

#### 6.4.6 「退出系统」按钮的处理流程

等价于托盘菜单的「退出系统」（参见 6.3.6）：
1. `MessageBox.Show(..., YesNoCancel, Question)`
   - 是：保存并重启服务再退
   - 否：不保存直接退
   - 取消：返回窗体
2. 退出前 `StopService("Api")` + `StopService("Admin")` + `Thread.Sleep(1000)`
3. `_notifyIcon.Visible = false` + `Application.Exit()`

#### 6.4.7 「数据库连接深度验证」按钮（增强版测试连接）

> **本节是 v2.6 增强**：原 6.4.2 字段清单中仅有"测试连接"按钮（仅 TCP+SQL 登录双验证），本节扩展为包含**读写操作测试**，确保数据库不仅可达，且当前账号对目标库有完整 DML/DDL 权限。同时增加**数据库库名称**字段（之前隐藏在连接字符串中）。
...
│                                                                    │
│ 服务器：[192.168.1.237___]   端口：[1433]                          │
│ 账号：[__DB_USER________]       密码：[●●●●●●__________]            │
│ 库名：[DormManageDB____]   ← 🆕 独立字段（之前在连接串中）        │
│                                                                    │
│       [🔌 测试连接] [🧪 读写测试] [🔄 重置]                        │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| 服务器 | TextBox | 是 | IP / 主机名 / `host,port` / `host\instance` |
| 端口 | NumericUpDown | 是 | 1–65535，SQL Server 默认 1433 |
| 账号 | TextBox | 是 | 数据库账号 |
| 密码 | PasswordChar | 是 | 数据库密码 |
| **库名** | TextBox | 是 | 🆕 目标数据库名（之前隐藏在连接串中） |
| 测试连接 | Button | — | 仅验证 TCP + 登录（快速） |
| 读写测试 | Button | — | 🆕 验证完整 DML/DDL 权限（深度） |
| 重置 | Button | — | 清空所有数据库字段 |

**6.4.7.2 测试流程对比**

| 测试类型 | 验证内容 | 耗时 | 失败时的典型问题 |
|---------|---------|------|----------------|
| **测试连接**（浅层） | ①TCP 端口可达<br/>②SQL Server 登录成功<br/>③可访问目标库 | < 3 秒 | 网络不通 / 账号密码错误 / 库不存在 |
| **读写测试**（深度） | ①通过"测试连接"的所有验证<br/>②尝试 CREATE TABLE 临时表<br/>③尝试 INSERT 一行测试数据<br/>④尝试 SELECT 验证写入<br/>⑤尝试 UPDATE 验证更新<br/>⑥尝试 DELETE 验证删除<br/>⑦DROP TABLE 清理测试数据<br/>⑧SELECT sys.databases 验证库可见 | 5–10 秒 | 账号权限不足（db_reader / db_writer） |

**6.4.7.3 「测试连接」按钮的处理流程（保留原版）**

```
点击 [🔌 测试连接]
  ↓
1. 解析"服务器"+"端口" → TCP 地址
2. TcpClient.BeginConnect(server, port, 10s 超时)
3. 失败 → 显示"网络错误：无法连接 192.168.1.237:1433（10s 超时）"
   ├─ 检查清单：SQL Server 已启动？TCP/IP 协议启用？防火墙放行？
4. 成功 → SqlConnection.Open()
5. 失败 → 显示"登录失败：<错误消息>"
   ├─ 检查清单：账号密码正确？账号未被锁定？
6. 成功 → SELECT name FROM sys.databases WHERE name = @dbName
7. 库不存在 → 显示"数据库 'DormManageDB' 不存在"
8. 库存在 → 显示"✅ 连接成功！服务器：192.168.1.237，数据库：DormManageDB"
```

**6.4.7.4 「读写测试」按钮的处理流程（🆕 深度验证）**
...
1. 调用"测试连接"流程（前置条件）
   ├─ 失败 → 直接返回错误，跳过 2-7 步
2. 连接成功后执行以下原子操作（SQL Server 专属）：
```

```sql
-- 创建测试表
CREATE TABLE wm_test_conn_20260710 (
    Id INT PRIMARY KEY,
...
-- 插入测试行
INSERT INTO wm_test_conn_20260710 (Id, TestName) VALUES (1, 'DormManage Test');

-- 验证读取
SELECT TestName FROM wm_test_conn_20260710 WHERE Id = 1;

-- 验证更新
UPDATE wm_test_conn_20260710 SET TestName = 'Updated' WHERE Id = 1;

-- 验证删除
DELETE FROM wm_test_conn_20260710 WHERE Id = 1;

-- 清理测试表
DROP TABLE wm_test_conn_20260710;

-- 验证库可见
SELECT name FROM sys.databases WHERE name = 'DormManageDB';
```

```
  ↓
3. SQLite 等价测试（如果是 SQLite 数据库）：
...
4. 全部成功 → 显示"✅ 读写测试通过！"
   ...
   └────────────────────────────────────────────┘
  ↓
5. 任何一步失败 → 显示具体错误（带 SQL 语句行号 + 错误码）：
   ┌────────────────────────────────────────────┐
   │  ❌ 读写测试失败                              │
   │                                              │
   │  失败步骤：UPDATE 操作                       │
   │  SQL 错误：The UPDATE permission was denied │
   │           on the object 'wm_test_conn_...'   │
   │  SQL 错误码：23000 (SQL Server)              │
   │                                              │
   │  解决方案：                                  │
   │  当前账号 '__DB_USER__' 缺少 UPDATE 权限。     │
   │  请数据库管理员执行：                        │
   │    GRANT UPDATE ON DATABASE::DormManageDB   │
   │      TO [__DB_USER__];                        │
   │                                              │
   │  已自动清理测试表（如果创建成功）            │
   └────────────────────────────────────────────┘
...
**6.4.7.5 DbTestHelper 完整代码实现**

```csharp
// 06-TrayApp/DbTestHelper.cs（v2.6 增强版）
public static class DbTestHelper
{
    /// <summary>测试结果记录</summary>
    ...
    /// <summary>
    /// 完整测试：TCP + 登录 + 库可见 + 读写操作
    /// </summary>
    public static TestResult FullTest(string server, int port, string dbName,
                                     string user, string password, int timeoutSec = 30)
    {
        ...
        {
            // Step 1: TCP 端口可达
            var tcpStep = TestTcpConnection(server, port, 10);
            result.Steps.Add(tcpStep);
            if (!tcpStep.Success)
            {
                result.ErrorStep = "TCP 连接";
                result.ErrorMessage = tcpStep.Detail;
                return CompleteResult(result, sw);
            }

            // Step 2: 构建连接串
            var connStr = BuildConnectionString(server, port, dbName, user, password);
            using var conn = new SqlConnection(connStr);
            conn.Open();

            // Step 3: 验证库可见
            var dbStep = TestDatabaseExists(conn, dbName);
            result.Steps.Add(dbStep);
            if (!dbStep.Success)
            {
                result.ErrorStep = "数据库可见性";
                result.ErrorMessage = dbStep.Detail;
                return CompleteResult(result, sw);
            }

            // Step 4-9: 读写测试（DDL + DML）
            var tableName = $"wm_test_conn_{DateTime.Now:yyyyMMddHHmmss}";

            result.Steps.Add(ExecuteSqlStep(conn, "CREATE TABLE", 
                $"CREATE TABLE [{tableName}] (Id INT PRIMARY KEY, TestName NVARCHAR(50), TestTime DATETIME DEFAULT GETDATE())"));

            result.Steps.Add(ExecuteSqlStep(conn, "INSERT",
                $"INSERT INTO [{tableName}] (Id, TestName) VALUES (1, 'DormManage Test')"));

            result.Steps.Add(ExecuteScalarStep(conn, "SELECT", 
                $"SELECT COUNT(*) FROM [{tableName}] WHERE Id = 1"));

            result.Steps.Add(ExecuteSqlStep(conn, "UPDATE",
                $"UPDATE [{tableName}] SET TestName = 'Updated' WHERE Id = 1"));

            result.Steps.Add(ExecuteSqlStep(conn, "DELETE",
                $"DELETE FROM [{tableName}] WHERE Id = 1"));

            // 清理测试表（即使前面失败也要清理）
            try { ExecuteSqlStep(conn, "DROP TABLE", $"DROP TABLE [{tableName}]"); }
            catch { /* 忽略清理失败 */ }
            ...
            if (failedStep != null)
            {
                result.ErrorStep = failedStep.Name;
                result.ErrorMessage = failedStep.Detail;
            }
        }
        catch (SqlException sqlEx)
        {
            result.ErrorMessage = sqlEx.Message;
            result.SqlErrorCode = sqlEx.Number;
            result.ErrorStep = "SQL 执行";
        }
        catch (Exception ex)
        {
            result.ErrorMessage = ex.Message;
            result.ErrorStep = "未知异常";
        }

        return CompleteResult(result, sw);
    }

    /// <summary>仅 TCP 端口探测（保留原浅层测试）</summary>
    public static TestStep TestTcpConnection(string server, int port, int timeoutSec)
    {
        var sw = Stopwatch.StartNew();
        try
        {
            using var client = new TcpClient();
            var ar = client.BeginConnect(server, port, null, null);
            if (!ar.AsyncWaitHandle.WaitOne(TimeSpan.FromSeconds(timeoutSec)))
                return Fail(sw, $"无法连接 {server}:{port}（{timeoutSec}s 超时）");
            client.EndConnect(ar);
            return Ok(sw, $"{server}:{port}");
        }
        catch (Exception ex)
        {
    ...
    private static TestStep TestDatabaseExists(SqlConnection conn, string dbName)
    {
        var sw = Stopwatch.StartNew();
        ...
        if (count == 0) return Fail(sw, $"数据库 '{dbName}' 不存在");
        return Ok(sw, $"数据库 '{dbName}' 可见");
    }

    private static TestStep ExecuteSqlStep(SqlConnection conn, string name, string sql)
    {
        var sw = Stopwatch.StartNew();
        try
        {
            ...
            cmd.CommandText = sql;
            cmd.ExecuteNonQuery();
            return Ok(sw, $"{name} 成功");
        }
        catch (Exception ex)
        {
            return Fail(sw, $"{name} 失败：{ex.Message}");
        }
    }

    private static TestStep ExecuteScalarStep(SqlConnection conn, string name, string sql)
    {
        var sw = Stopwatch.StartNew();
        try
        {
    ...
    private static TestResult CompleteResult(TestResult r, Stopwatch sw)
    {
        sw.Stop();
        r.ElapsedMs = sw.ElapsedMilliseconds;
        r.Success = r.Steps.All(s => s.Success);
        return r;
    }

    private static TestStep Ok(Stopwatch sw, string detail) => new()
    { Success = true, ElapsedMs = sw.ElapsedMilliseconds, Detail = detail };

    private static TestStep Fail(Stopwatch sw, string detail) => new()
    { Success = false, ElapsedMs = sw.ElapsedMilliseconds, Detail = detail };
}
```

**6.4.7.6 SettingsForm 中调用 DbTestHelper**

```csharp
// 06-TrayApp/SettingsForm.cs 中新增事件
private void DoTestConnection()  // 🔌 浅层（仅 TCP + 登录）
{
    var cfg = CollectDbConfig();
    var sw = Stopwatch.StartNew();
    var result = DbTestHelper.FullTest(cfg.Server, cfg.Port, cfg.DbName, 
                                       cfg.User, cfg.Password, timeoutSec: 10);
    sw.Stop();

    if (result.Success)
    {
        MessageBox.Show(
            $"✅ 连接成功！\n\n服务器：{cfg.Server}:{cfg.Port}\n数据库：{cfg.DbName}\n账号：{cfg.User}\n\n耗时：{sw.ElapsedMilliseconds}ms",
            "测试连接", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
    else
    {
        MessageBox.Show(
            $"❌ 连接失败\n\n失败步骤：{result.ErrorStep}\n错误信息：{result.ErrorMessage}",
            "测试连接", MessageBoxButtons.OK, MessageBoxIcon.Error);
    }
}

private void DoReadWriteTest()  // 🧪 深度（含读写测试）
{
    var cfg = CollectDbConfig();
    
    // 确认弹窗（避免误操作）
    if (MessageBox.Show(
        $"即将对数据库 '{cfg.DbName}' 执行 8 项深度测试：\n\n" +
        "  ✓ TCP 端口探测\n" +
        "  ✓ SQL Server 登录\n" +
        "  ✓ 数据库可见性\n" +
        "  ✓ CREATE TABLE 临时表\n" +
        ...
        "  ✓ DROP TABLE 清理\n\n" +
        "确认开始？",
        "读写测试", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
        return;

    _btnTestDbReadWrite.Enabled = false;
    _btnTestDbReadWrite.Text = "测试中...";
    Application.DoEvents();  // 让 UI 立即更新
    
    var result = DbTestHelper.FullTest(cfg.Server, cfg.Port, cfg.DbName,
                                       cfg.User, cfg.Password);

    _btnTestDbReadWrite.Enabled = true;
    _btnTestDbReadWrite.Text = "🧪 读写测试";

    // 显示详细结果
    var sb = new StringBuilder();
    sb.AppendLine(result.Success ? "✅ 数据库连接 + 读写测试全部通过" : "❌ 读写测试失败");
    sb.AppendLine();
    sb.AppendLine($"服务器：{cfg.Server}:{cfg.Port}");
    sb.AppendLine($"数据库：{cfg.DbName}");
    sb.AppendLine($"账号：{cfg.User}");
    sb.AppendLine();
    sb.AppendLine("测试步骤：");
    foreach (var step in result.Steps)
    {
        sb.AppendLine($"  {(step.Success ? "✓" : "✗")} {step.Name,-15} ({step.ElapsedMs}ms)");
        if (!string.IsNullOrEmpty(step.Detail))
            sb.AppendLine($"      {step.Detail}");
    }
    sb.AppendLine();
    sb.AppendLine($"总耗时：{result.ElapsedMs}ms");
    
    if (!result.Success)
    {
        sb.AppendLine();
        sb.AppendLine($"失败步骤：{result.ErrorStep}");
        sb.AppendLine($"错误信息：{result.ErrorMessage}");
        if (result.SqlErrorCode.HasValue)
            sb.AppendLine($"SQL 错误码：{result.SqlErrorCode}");
    }
...
private DbConfig CollectDbConfig() => new()
{
    Server = _txtDbServer.Text.Trim(),
    Port = (int)_nudDbPort.Value,
    DbName = _txtDbName.Text.Trim(),
    User = _txtDbUser.Text.Trim(),
...
**6.4.7.7 与 6.5 双 UI 协同的扩展**

根据方案 v2.2 的双 UI 协同原则，数据库测试功能在两个 UI 中都可用：

| UI | 入口 | 按钮位置 | 测试深度 |
|----|------|---------|---------|
| **托盘菜单** | 「系统配置」→「数据库」Tab | `[🔌 测试连接]` | 浅层（TCP + 登录） |
| **托盘菜单** | 「系统配置」→「数据库」Tab | `[🧪 读写测试]` | 🆕 深度（含读写） |
| **Web 后台** | 「/Admin/Config」→「数据库」 | 「测试连接」按钮 | 浅层（保持原版） |
| **Web 后台** | 「/Admin/Config」→「数据库」 | 🆕 「读写测试」按钮（新增） | 🆕 深度 |

**6.4.7.8 错误码速查表**

| SQL Server 错误码 | 含义 | 解决方案 |
|------------------|------|---------|
| 18456 | 登录失败（账号密码错） | 检查账号密码 |
| 4060 | 无法打开数据库 | 确认数据库存在 |
| 262 | 权限不足（CREATE TABLE 拒绝） | GRANT CREATE TABLE 权限 |
| 229 | 权限不足（INSERT/UPDATE/DELETE 拒绝） | GRANT INSERT/UPDATE/DELETE 权限 |
| 3701 | 表已存在（冲突） | 重新运行测试（带时间戳表名） |
| 208 | 对象名无效（SQL 拼写错误） | 检查 SQL 语句 |
...
### 6.5 服务启停双 UI 协同设计（核心章节）

> **本章定位（v2.11.7 修订）**：明确「托盘菜单」与「系统配置窗口」是**核心服务端运维的双 UI 入口**，共享底层 `ProcessManager`，通过事件驱动保持状态同步。系统配置窗口仅保留核心服务端参数配置（端口/数据库/图片路径/服务启停/保存参数），**无角色权限控制**；Web 端系统设置承载全部业务配置功能（用户角色/备份恢复/系统集成/筛选缓存等），**受角色权限管控**。两者从"功能互补"调整为"职责分离"。

#### 6.5.1 双 UI 设计哲学

托盘管理程序为服务控制功能提供**两套等价入口**，背后共享同一套业务逻辑：

| UI 入口 | 适用场景 | 用户角色 | 信息密度 |
|---------|---------|---------|---------|
| **托盘菜单**（参见 6.3） | 紧急启停、远程桌面、远程协助、屏幕小空间 | 运维工程师、系统管理员 | **极简**（仅 6 项） |
| **系统配置窗口**（参见 6.4） | 核心服务端参数配置（端口/数据库/图片路径/服务启停/保存） | **无权限控制**，本地任意用户 | **精简**（仅 16 个控件） |

**双 UI 协同的三大原则**：

1. **功能等价（Equivalence）**：托盘菜单的「启动/停止 PDA」与系统配置窗口中的「启动/停止 PDA 按钮」**调用同一个方法、共享同一个状态机、产生完全一致的业务效果**。
2. **状态一致（Consistency）**：无论用户从哪个入口触发操作，所有 UI 都要实时反映最新状态（事件驱动）。
3. **职责分离（Separation of Concerns）**：**托盘窗口仅负责核心服务端参数**（端口/数据库/图片路径/服务启停/保存），**Web 端负责全部业务配置**（用户角色/备份恢复/系统集成/筛选缓存等）并受角色权限管控。

> **v2.11.7 变更**：托盘窗口从"完整配置"收敛为"核心服务端参数"，移除 APP下载、打开网址等非核心功能，这些功能已迁移至 Web 端系统设置模块。

#### 6.5.2 功能矩阵：双 UI 入口对照

下表列出所有可执行操作的双 UI 入口对照：

| # | 操作 | 托盘菜单入口 | 系统配置窗口入口 | 共享业务方法 | 备注 |
|---|------|------------|----------------|------------|------|
| 1 | **启动 PDA 服务** | ✅ 菜单项「启动PDA服务」 | ✅ 「启动」按钮（PDA 行） | `ProcessManager.StartService("Api")` | 完全等价 |
| 2 | **停止 PDA 服务** | ✅ 菜单项「停止PDA服务」 | ✅ 「停止」按钮（PDA 行） | `ProcessManager.StopService("Api")` | 完全等价 |
| 3 | **启动 WEB 服务** | ✅ 菜单项「启动WEB服务」 | ✅ 「启动」按钮（Web 行） | `ProcessManager.StartService("Admin")` | 完全等价 |
| 4 | **停止 WEB 服务** | ✅ 菜单项「停止WEB服务」 | ✅ 「停止」按钮（Web 行） | `ProcessManager.StopService("Admin")` | 完全等价 |
| 5 | **修改 PDA 端口号** | ❌ | ✅ 「PDA服务端口号」NumericUpDown | `ApplyPort(...)` + `SaveConfig(...)` | 配置窗口独有 |
| 6 | **修改 Web 端口号** | ❌ | ✅ 「Web服务端口号」NumericUpDown | `ApplyPort(...)` + `SaveConfig(...)` | 配置窗口独有 |
| 7 | **修改图片保存路径** | ❌ | ✅ 「图片保存路径」TextBox | `ConfigService.SaveConfig(...)` | 配置窗口独有 |
| 8 | **修改数据库连接** | ❌ | ✅ 「数据库」行 4 个字段 | `ConfigService.SaveConfig(...)` | 配置窗口独有 |
| 9 | **修改服务器域名** | ❌ | ✅ 「服务器域名」TextBox | `ConfigService.SaveConfig(...)` | 配置窗口独有 |
| 10 | **保存配置** | ❌ | ✅ 「保存」按钮 | `ConfigService.SaveConfig(...)` | 配置窗口独有 |
| 11 | **打开系统配置窗口** | ✅ 菜单项「系统配置」 | — | `ShowSettingsOnUiThread()` | 托盘菜单独有 |
| 12 | **退出托盘程序** | ✅ 菜单项「退出系统」 | ✅ 「退出系统」按钮 | `Application.Exit()` | 完全等价 |

**关键洞察**：
- **「启动/停止 PDA / WEB」这 4 个操作在两处都有入口**，是双 UI 协同的核心场景。
- **「核心服务端参数配置」**（端口、路径、数据库、域名、保存）只在配置窗口提供，因为修改这些需要看到完整字段。
- **「业务配置类」操作**（PDA 版本管理、用户角色、备份恢复、系统集成、筛选缓存等）**全部在 Web 端实现**，受角色权限管控。
- **「托盘程序级」操作**（打开配置窗口、退出托盘）由托盘菜单独占入口。
...
```
                ...
                │   - StopService(name)            │
                │   - RestartService(name)         │
                │   + StatusChanged 事件 ◄─────────┼─── 唯一的"事实源"
                └──────────────┬───────────────────┘
                               │ 状态变化事件
              ┌────────────────┼────────────────┐
              ▼                                 ▼
   ┌────────────────────┐         ┌────────────────────────┐
   │  TrayIconManager   │         │   SettingsForm         │
   │  (托盘菜单)        │         │   (系统配置窗口)        │
   ├────────────────────┤         ├────────────────────────┤
   │ 订阅 StatusChanged │         │ 订阅 StatusChanged     │
   │ → UpdateMenuState  │         │ → RefreshStatus        │
   │   ├─ 启/停项Enabled│         │   ├─ 启/停按钮 Enabled │
   │   ├─ 文字联动      │         │   ├─ 状态文字更新      │
   │   └─ 托盘图标颜色  │         │   └─ 状态颜色更新      │
   └────────────────────┘         └────────────────────────┘
```
...
**关键约定**：
- `ProcessManager` 是**唯一的状态真相源**（Single Source of Truth）。
- UI 层（托盘菜单 + 配置窗口）**只订阅事件，不主动查询状态**（除非首次初始化）。
- 用户操作 → 调用 `ProcessManager` 方法 → 修改内部状态 → 触发 `StatusChanged` 事件 → UI 自动刷新。
- 事件订阅与反订阅：UI 关闭时（如 `SettingsForm.FormClosed`）必须 `-= StatusChanged`，防止内存泄漏。

**核心代码骨架**：

```csharp
// ProcessManager.cs —— 唯一的状态机
public class ProcessManager : IDisposable
{
    public event Action<ServiceStatusChange>? StatusChanged;

    public bool StartService(string serviceName)
    {
        // 修改 _services[serviceName].Status
        // 触发 StatusChanged 事件
        RaiseStatusChanged(serviceName, info);
        return true;
    }
...
// SettingsForm.cs —— 订阅事件 → 刷新窗口内控件
_processManager.StatusChanged += OnStatusChanged;
FormClosed += (_, _) => _processManager.StatusChanged -= OnStatusChanged;

private void OnStatusChanged(ProcessManager.ServiceStatusChange c)
{
...
#### 6.5.4 实现架构（共享 ProcessManager 单例）

托盘程序启动时创建 **唯一一个** `ProcessManager` 实例，托盘菜单与配置窗口共享同一实例：

```
Program.Main()
    │
    ├─ new ConfigService(exeDir)
    ├─ new GuardianLog()
    ├─ new ProcessManager(configService, guardianLog)  ◄─── 单例，全局共享
    │
    ├─ new TrayIconManager(processManager, configService, guardianLog)
    │     └─ 订阅 StatusChanged
    │
    └─ Application.Run(new MainForm())
         └─ 用户点击托盘菜单「系统配置」→ ShowSettingsOnUiThread()
              └─ new SettingsForm(processManager, configService)  ◄─── 接收同一实例
                   └─ 订阅 StatusChanged
```

**关键约束**：
- `ProcessManager` 实例在 `Program.Main` 中创建，生命周期与托盘进程一致。
- `TrayIconManager` 与 `SettingsForm` 通过构造函数参数注入**同一实例**，不创建副本。
- `SettingsForm` 可多次打开/关闭，每次 `new SettingsForm(...)` 都接收同一 `ProcessManager`，因此多窗口（如未来支持）状态也能同步。
- 事件订阅使用 `Action<...>` 委托，注意在 `FormClosed` 时 `-=` 取消订阅，避免泄漏。

#### 6.5.5 操作流程对比（托盘菜单 vs 配置窗口）

以「启动 PDA 服务」为例对比两种入口的完整流程：

**入口 A：托盘菜单**
```
1. 用户右键托盘图标
2. 弹出菜单 → 点击「启动PDA服务」
3. _processManager.StartService("Api")
4. ProcessManager:
   a. info.Status = Starting → 触发 StatusChanged
   b. 启动 EXE 进程（Process.Start）
   c. 启动 30s 健康检查 Task
5. TrayIconManager 收到事件：
   a. UpdateMenuState → 「启动PDA服务」Disabled / 「停止PDA服务」Enabled
   b. 托盘图标颜色变化（Gray → Gold）
6. 30s 内健康检查通过：
   a. info.Status = Running → 触发 StatusChanged
   b. 托盘图标 → Green
7. 如果 SettingsForm 当前打开：
   a. 自动刷新窗口内按钮 Enabled + 状态文字 "运行中"
```

**入口 B：系统配置窗口**
```
1. 用户右键托盘图标 → 菜单「系统配置」 → SettingsForm 打开
2. 用户看到 PDA 行：[端口 5000] [启动] 运行中
3. 用户点击 [启动] 按钮
4. _processManager.StartService("Api")
5. ProcessManager:
   a. info.Status = Starting → 触发 StatusChanged
   b. 启动 EXE 进程
   c. 启动 30s 健康检查
6. SettingsForm 收到事件：
   a. RefreshStatus → PDA 行按钮 Disabled、文字变 "启动中..."
7. 30s 内健康检查通过：
   a. info.Status = Running → 触发 StatusChanged
   b. SettingsForm 文字变 "运行中"、按钮状态更新
8. 如果托盘菜单当前显示：
   a. 自动刷新菜单项 Enabled 状态
```

**两种入口的唯一差异**：
- 入口 A 不打开配置窗口（屏幕占用小，适合远程协助）
- 入口 B 在配置窗口内操作（屏幕占用大，但提供完整字段可视化）

**业务效果完全等价**：最终都是调用同一个 `StartService("Api")`，产生完全相同的进程启动逻辑、健康检查逻辑、状态变化。

#### 6.5.6 双 UI 设计要点（10 条规范）

| # | 规范 | 说明 |
|---|------|------|
| 1 | **共享底层，永不复制** | 托盘菜单与配置窗口调用同一 `ProcessManager` 方法，绝不各自实现启停逻辑 |
| 2 | **事件驱动，禁用轮询** | UI 刷新全靠 `StatusChanged` 事件订阅，禁止 `Timer` 轮询状态 |
| 3 | **跨线程必 Invoke** | 事件回调可能在非 UI 线程，必须 `Invoke` / `BeginInvoke` 到 UI 线程 |
| 4 | **关闭时反订阅** | `SettingsForm.FormClosed` 必须 `-= StatusChanged`，防止内存泄漏 |
| 5 | **视觉同步** | 同一操作的两入口状态显示文字、颜色必须一致（如"运行中"统一显示） |
| 6 | **操作可逆** | 「启动」与「停止」必须在所有状态下互斥，但都需根据当前状态动态 Enabled |
| 7 | **错误统一处理** | 启动失败（如 EXE 文件丢失）在两处都要弹窗告知用户，不能静默失败 |
| 8 | **避免重复操作** | 双 UI 入口都可能在「Starting」状态被点击，必须禁用按钮防重复 |
| 9 | **保存与启停解耦** | 配置窗口的「保存」只写配置不重启；「启动/停止」按钮不写配置（解耦） |
| 10 | **退出统一处理** | 托盘菜单「退出系统」与配置窗口「退出系统」等价，均需二次确认 |

#### 6.5.7 双 UI 协同测试要点

| 测试场景 | 验证内容 |
|---------|---------|
| **场景 1：托盘菜单启动 PDA** | 点击后 30s 内托盘图标变绿；打开配置窗口后状态文字"运行中" |
| **场景 2：配置窗口启动 WEB** | 点击按钮后菜单项联动；托盘图标颜色同步更新 |
| **场景 3：托盘菜单停止 PDA** | 点击后状态变 Stopped；打开配置窗口后按钮回到初始状态 |
| **场景 4：同时打开多个配置窗口**（未来支持） | 两窗口状态完全一致，无状态冲突 |
| **场景 5：健康检查失败** | 托盘图标变红；配置窗口显示"异常"；菜单项按钮回到 Stopped 状态可重新启动 |
| **场景 6：关闭窗口后再操作** | 关闭 SettingsForm 后从托盘菜单触发启停，托盘菜单状态依然正常刷新 |
| **场景 7：保存配置触发重启** | 配置窗口保存后选择「是，重启」，所有 EXE 重启完成状态正确 |
...
使用全局命名 Mutex 防止重复启动：
```csharp
private const string MutexName = "Global\\DormManage.TrayApp.SingleInstance.v1";
_singleInstanceMutex = new Mutex(initiallyOwned: true, MutexName, out bool createdNew);
if (!createdNew) {
    MessageBox.Show("监控程序已在运行！\n请检查任务栏右下角托盘图标。");
    return;
}
```
...
通过 `Process.Start` 启动 Api / Admin EXE，注入环境变量：
```csharp
var psi = new ProcessStartInfo {
    FileName = exePath,
    UseShellExecute = true,
...
- `DormManage_CONFIG_FILE`：子进程优先从此路径加载配置（替代默认查找）

### 6.8 启动期健康检查（30s 黄金窗口）

```csharp
// ProcessManager.StartService() 内
Task.Run(async () => {
    for (int i = 0; i < 15 && info.Status == ServiceStatus.Starting; i++) {
        try {
        ...
        await Task.Delay(2000);  // 每 2s 探测一次
    }
    // 30s 内未通过 → 标记为 Error
...
| 配置项 | 说明 |
|--------|------|
| **PDA 服务端口** | 1–65535，默认 5000，含启动/停止按钮 |
| **Web 服务端口** | 1–65535，默认 8080，含启动/停止按钮 |
...
| 图片文件 | `D:\MeterImages\` | **P0** | 每日增量 + 每周全量 |
...
robocopy D:\MeterImages \\NAS\DormManageImages\Weekly /MIR /R:3 /W:10 /LOG:D:\BackupLogs\images_$(date).log
...
# 4. 还原图片
robocopy \\NAS\DormManageImages\Latest D:\MeterImages /MIR
...
**关键**：`Jwt:Secret` 必须 ≥ 32 字符随机串，生产环境使用 `dotnet user-secrets` 或 Windows DPAPI 加密存储。
...
**现象**：上传返回 500，日志 `Access to the path 'D:\MeterImages\...' is denied`。

**解决**：
- 直接双击 EXE 启动 → 当前 Windows 用户即拥有权限，无需额外配置
- 若通过任务计划程序 / 服务方式启动 → 配置任务时选"使用当前用户登录"或"最高权限运行"
- 服务方式需指定专用服务账号，并对 `D:\MeterImages` 授予"修改"权限
...
**解决**：
- 检查 `netstat -ano | findstr :5000`，找到占用进程 PID
- 任务管理器结束或通过托盘修改端口
...
const BASE_URL = 'http://192.168.1.237:5000';
let token = '';
...
| **Base URL** | `http://{ServerDomain}:5000`（生产） / `http://10.0.2.2:5000`（Android 模拟器） |
...
Response 200 (成功):
{
    ...
    "data": {
        ...
        "readMonth": "2026-07",
        "imageUrls": [
            "http://server:8080/uploads/202607/D-301/20260709_143055_cold.jpg",
            "http://server:8080/uploads/202607/D-301/20260709_143055_hot.jpg",
            "http://server:8080/uploads/202607/D-301/20260709_143055_electric.jpg"
        ],
        "duplicate": false
    }
}
...
{
    "items": [
        { "key": "image_base_path", "value": "D:\\MeterImages" },
...
app.UseSwaggerUI(c => {
    ...
    c.RoutePrefix = "api-docs";  // 访问 http://server:5000/api-docs
});
```

**访问地址**：`http://server:5000/api-docs`（开发环境）/ `http://server:5000/api-docs`（生产环境，仅内网）
    ...
    "data": [
        {
            ...
            "systemCode": "HR",
            "systemName": "人力资源系统",
            "serverUrl": "http://hr.company.com:8080/api",
      ...
      - name: Health Check
        run: |
          ...
          for ($i = 1; $i -le $maxRetry; $i++) {
              $status = (Invoke-WebRequest -Uri "http://$env:PROD_SERVER:5000/health" -UseBasicParsing).StatusCode
              if ($status -eq 200) {
                  Write-Host "✓ 服务已就绪"
                  exit 0
              }
...
| **v2.11.7** | **2026-07-12** | **托盘系统配置窗口职责收敛：SettingsForm 仅保留核心服务端参数（PDA/Web 端口、数据库连接、图片存储路径、服务器域名、服务启停、保存参数），无角色权限项控制；Web 端系统设置承载全部功能（服务管理、数据库、PDA版本、用户角色、备份恢复、系统集成、筛选缓存），受角色权限管控；移除 APP下载/打开网址按钮；双 UI 从"完整配置互补"调整为"托盘轻量运维 + Web 完整管理"** |
...
| **v2.11.6** | **2026-07-12** | **新增「系统集成」子功能：SysSystemIntegration 数据库表（SystemCode/SystemName/ServerUrl/Username/PasswordHash/AES-256加密/Enabled/LastTestResult）+ 4 个系统集成 API 端点（GET/PUT/POST 测试）+ 系统设置原型新增"系统集成"Tab + HR/K3ERP 双系统配置（服务器地址/账号/密码/启用状态/连接测试/保存参数）** |
| **v2.11.7** | **2026-07-12** | **托盘系统配置窗口职责收敛：SettingsForm 仅保留核心服务端参数配置（PDA/Web 端口、数据库连接、图片存储路径、服务启停、保存参数），无角色权限项控制；Web 端系统设置承载全部功能（服务管理、数据库、PDA版本、用户角色、备份恢复、系统集成、筛选缓存），受角色权限管控；双 UI 从"完整配置互补"调整为"托盘轻量运维 + Web 完整管理"** |
...
| **v2.12.36** | **2026-07-14** | **新增 §3.6.10「系统启动顺序与运行期控制规范」核心章节：明确 6 阶段标准化启动顺序（托盘先启动 → Web 启动期连接数据库失败即停止 → PDA 启动期连接数据库失败即停止 → 数据库参数在线修改 → 系统重连 → 守护监测）；新增数据库连接串本地加密存储方案（Windows DPAPI，`DataProtectionScope.CurrentUser`，零密钥管理、跨用户/跨机器不可移植），澄清"MD5 加密/解密"技术误区；新增托盘主日志目录规范（`%LOCALAPPDATA%\DormManage\Log/YYYYMMDD_log.txt`）与双层监测机制（`GuardianTimer` 2s 进程扫描 + `HealthCheckTimer` 10s 端点健康）；新增 3 种停止/退出路径（停 PDA / 停 Web / 退出系统）与二次确认流程；与 §3.6.1、§6、§8.1、§9.1、§11.2 保持一致** |

---

> **本方案（v2.11.7）系统性地将服务端架构由 IIS 反向代理重构为 .NET 8 自托管 EXE + 托盘管理程序守护，并配套三层进程防护、可视化配置（含数据库深度验证）、人员清单管理、费用管理（费用标准+宿舍账单+员工分摊）、日志审计、备份恢复、监控运维、安全合规等完整工程实践。所有部署、运维、升级均基于 EXE 进程模型，不再依赖 IIS。**
>
> **v2.11.7 确立托盘系统配置窗口职责收敛：SettingsForm 仅保留核心服务端参数配置（PDA/Web 端口、数据库连接、图片存储路径、服务器域名、服务启停、保存参数），无角色权限项控制；Web 端系统设置承载全部功能（服务管理、数据库、PDA版本、用户角色、备份恢复、系统集成、筛选缓存），受角色权限管控；双 UI 从"完整配置互补"调整为"托盘轻量运维 + Web 完整管理"。**
>
> **v2.5 完成 PDA APP 分发闭环：Gradle assembleRelease 构建 APK → 标准化命名（pda-app-{version}-{code}-{flavor}-{date}.apk）→ 部署到 Admin wwwroot/downloads/ → Kestrel UseStaticFiles 托管并设置 APK 标准 MIME（application/vnd.android.package-archive）→ PC/PDA 浏览器通过 http://server:8080/downloads/*.apk 下载 → Android 系统安装到 PDA 设备。配合系统配置窗口「APP下载」按钮、目录浏览、自动更新机制，形成从开发者到抄表员的无缝分发链路。**
>
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  🏭 金戈水电抄表  [首页] [记录] [统计] [人员] [宿舍] [费用] [系统]  admin│
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
