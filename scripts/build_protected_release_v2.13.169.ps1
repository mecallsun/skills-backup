# v2.13.169 一键构建受保护发布包（程序启动注册校验机制重构 —— 硬阻断 → 按状态降级）
#
# 基于 build_protected_release.ps1（v2.13.141 修复版：Obfuscar-only，BitMono 已禁用）
# 变更：版本号 → v2.13.169；输出名带时间戳；zip 保留 Admin/Api/TrayApp 目录结构；附带交付文档
#
# 保护链路：Obfuscar 全栈混淆 + PublishReadyToRun R2R + HideStrings + SuppressIldasm + IL 优化
# 关键变更（v2.13.169）：已删除 RuntimeWindowGuard/TamperDialog/UnlockSequenceBuffer 暗桩三件套；
# ProcessManager.StartAllAsync 删除 IsLicenseValid 硬阻断；LicenseStatusController 新增

$ErrorActionPreference = "Stop"
$ROOT = (Resolve-Path "$PSScriptRoot\..").Path
Set-Location $ROOT
$ts = Get-Date -Format "yyyyMMdd_HHmmss"
$out = "DormManage-v2.13.169_$ts.zip"
$logDir = Join-Path $ROOT "logs"
if (-not (Test-Path $logDir)) { New-Item -ItemType Directory -Path $logDir -Force | Out-Null }
$log = Join-Path $logDir "build_v2.13.169_$ts.log"

Write-Host "════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "  v2.13.169 一键构建受保护发布包" -ForegroundColor Cyan
Write-Host "  (启动校验重构 + 删暗桩 + RegStatus 4 态)" -ForegroundColor Cyan
Write-Host "════════════════════════════════════════════════" -ForegroundColor Cyan
...
# 附带交付文档到 _delivery 目录
$docDir = "publish-final/_delivery_v2.13.169"
if (Test-Path $docDir) { Remove-Item $docDir -Recurse -Force }
New-Item -ItemType Directory -Path $docDir -Force | Out-Null
Copy-Item "00-方案文档/210-程序启动注册校验机制重构-v2.13.169.md" $docDir/ -Force
