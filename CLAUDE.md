...
- 暂存(`git add`)时自动将 `<真实用户名>`→`__DB_USER__`、`<真实密码>`→`__DB_PASSWORD__` 入库；工作区文件保持真实值不变；git 比较脱敏后内容，故无 `modified` 噪声。
- 覆盖类型：`*.json / *.cs / *.md / *.sql / *.py / *.ps1`。
- 新增敏感值时：扩展 `filter.redactdb.clean` 的 sed 规则 + 确认 `.gitattributes` 覆盖该文件类型。

**克隆后必做**（过滤器为本地配置，不随仓库同步；将 `<真实用户名>`/`<真实密码>` 换为实际值）：
```bash
git config filter.redactdb.clean "sed -E 's/<真实密码>/__DB_PASSWORD__/g; s/<真实用户名>/__DB_USER__/g'"
...
**历史清理（v2.13.8 已执行）**：已用 `git filter-repo --replace-text` 将全部历史提交中的明文凭据替换为占位符并强推，历史 `git log -S` 检索明文命中数为 0。
