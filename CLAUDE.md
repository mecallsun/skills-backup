...
- **项目当前版本：** v2.13.65（2026-07-21 系统设置 用户管理 Index 占位符 P0 修复 + User.cshtml AJAX 化 + 删除并发异常修复 — Settings/Index tab-users / tab-roles 「原型演示」占位代码替换为引导到独立页面 `/Settings/User` / `/Settings/Role`；User.cshtml 全面 AJAX 化（Bootstrap 5 data-bs-toggle modal + data-* 属性传递 + fetch + 页面级 antiforgery token + JSON feedback）+ PageModel handler 改返回 JsonResult；删除操作改用 `ExecuteDeleteAsync` 直发 SQL 解决 `DbUpdateConcurrencyException`；4 个 CRUD 操作（Create / Update / ResetPwd / Delete）端到端 curl 测试全部 HTTP 200 + JSON success:true）
