...
- **项目当前版本：** v2.13.92（2026-07-22 系统设置 9 Tab 新增「字段权限」+ RBAC 四级权限（数据权限 PermissionType=3 落地）— 新表 `SysFieldPermission`（5 字段 seed：realname/phone/employeecode/dormcode/remark）+ 新权限码 `privacy:field:enable`（PermissionType=3，复用现有 RBAC）+ 7 页面 `Html.IsFieldHiddenAsync` 集成（Personnel/Index+Details、Booking/Index+CheckIn+CheckOut+Edit、EmployeeBilling/Details）+ Settings 9 Tab 字段权限面板 `_FieldPermissionPanel.cshtml` + 角色权限矩阵 Modal 顶部独立「隐私字段权限」checkbox + `FieldPermissionHtmlHelperExtensions` 渲染层扩展 + HttpContext.Items 缓存避免每列 N+1；详见 `00-方案文档/145-字段权限隐私保护-v2.13.92.md`）
...
- **v2.13.79**（2026-07-21 抄表记录 BIGINT 类型对齐 — MeterRecord.RecordId DB schema 为 BIGINT IDENTITY 但 EF 模型继承 BaseEntity.Id (int)，物化时 Int64 → Int32 cast 失败导致 /Meter 列表 Error。修复：`MeterRecord.cs` 添加 `public new long Id { get; set; }` + `[Column("RecordId")]`；`DormDbContext.cs` 移除 `HasColumnName("RecordId").HasColumnType("int")`；端到端验证 /Meter HTTP 200 + 338 条记录正确显示。详见 `00-方案文档/130-抄表记录BIGINT类型对齐-v2.13.79.md`）
- **v2.13.80**（2026-07-21 抄表记录手动实录 1:1 原型对齐 + 5 项 BUG 修复 — POST /api/meter/manual-entry HTTP 500（ClientRecordId NULL）+ DTO 缺字段 + NVARCHAR(128) vs DB NVARCHAR(64) + IsRequired 与 seed data 冲突 + UTF-8 BOM；详见 `00-方案文档/131-抄表记录手动实录1比1原型对齐-v2.13.80.md`）
- **v2.13.81**（2026-07-21 抄表记录「手动补录」弹窗独占模式 1:1 原型对齐 — 用户 2 次明确纠正「原型中手动补录是弹窗独占模式（data-bs-toggle="modal"），不是跳转独立页面」；修复 PageHeader 4 个按钮（移除「新增记录」/「手动补录」改 OnClick modal / 「批量导入」改 Url 独立页面 / 「导出Excel」改 Url 直连 API）；新增 #modalManualEntry（modal-xl 900px + data-bs-backdrop="static" data-bs-keyboard="false"）+ Index.cshtml.cs OnGetLoadReadings AJAX handler；端到端验证 4 个 LoadReadings 场景（空/有记录/effective）+ manual-entry POST 创建记录 id=12。详见 `00-方案文档/132-抄表记录手动补录弹窗独占模式1比1原型对齐-v2.13.81.md`）
