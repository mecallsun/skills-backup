...
- **项目当前版本：** v2.13.113（2026-07-22 入住信息员工信息 FK 显示修复 — 用户原话「员工选择 张 JG003032 张金文 部门: 类型: 班次: 中的部门显示员工部门id FK的部门分列名称；班次、班组也同样显示」；这是 v2.13.78（班组FK）→ v2.13.91/97/111（列表页班组列）共 4 个版本遗漏的「selectEmp 单个员工信息卡片」场景；6 处代码修复：① mock-data.js PERSONNEL 650 条记录补 teamId/teamName ② 原型 check-in.html/check-out.html selectEmp 部门用 DEPARTMENTS.find FK 查字典 + 加班组 Badge 渲染 + emp-info-card 容器加班组 span ③ BookingService.EmployeeSearchResult DTO 加 DepartmentId/DepartmentName/TeamId/TeamName ④ SearchEmployeeAsync LINQ 投影 JOIN Department 表 + Team 导航属性 ⑤ Admin CheckIn.cshtml/CheckOut.cshtml selectEmp 函数 DepartmentName/TeamName FK + 容器加班组 span；详见 `00-方案文档/165-入住信息员工信息FK显示修复-v2.13.113.md`）
