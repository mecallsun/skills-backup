...
- **项目当前版本：** v2.13.85（2026-07-21 性别字段全链路打通 — v2.13.83 行政资料 + 姓名推断批量回填 906 员工（男 752 + 女 154）；v2.13.84 办理入住性别约束（DormOption 扩展 + Service 层 DORM_GENDER_CONFLICT 兜底 + UI Badge）；v2.13.85 宿舍档案派生性别（取消手工维护，Dorms/Index + Details + Edit 全部改为 EffectiveGender 派生 Badge）；详见 `00-方案文档/134-人员清单性别字段补全与批量回填-v2.13.83.md` / `135-办理入住房间性别约束-v2.13.84.md` / `136-宿舍档案派生性别字段-v2.13.85.md`）
