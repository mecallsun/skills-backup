...
- **项目当前版本：** v2.13.67（2026-07-21 系统设置子 Tab 嵌入重组 — 删除 v2.13.65「独立页面」引导卡；用户管理 + 角色权限完整 CRUD 嵌入 /Settings 页面作为子 Tab，与「数据库连接」「备份」「PDA 版本」等其他子 Tab 并列；partial class IndexModel + 4 个 partial 文件 + 删除 4 个独立文件；端到端测试 Role + User 6 个 CRUD 操作全部 success）
