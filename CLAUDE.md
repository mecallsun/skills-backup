...
- **项目当前版本：** v2.13.81（2026-07-21 抄表记录「手动补录」弹窗独占模式 1:1 原型对齐 — 按用户 2 次明确纠正「弹窗 vs 独立页面」识别规则；PageHeader 4 按钮按原型重排：手动补录→Modal / 批量导入→独立页面 / 导出Excel→直连API / 移除「新增记录」；新增 #modalManualEntry modal-xl 900px + OnGetLoadReadings AJAX handler；详见 `00-方案文档/132-抄表记录手动补录弹窗独占模式1比1原型对齐-v2.13.81.md`）
