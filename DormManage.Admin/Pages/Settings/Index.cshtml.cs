...
public partial class IndexModel : PageModel
{
    ...
    public List<PermissionDto> PermissionMatrix { get; set; } = new()
    {
        ...
        new() { Module = "住宿管理", AdminAccess = true, FinanceAccess = false, PdaAccess = false },
        ...
        new() { Module = "住宿账单", AdminAccess = true, FinanceAccess = true, PdaAccess = false },
    ...
    public async Task OnGetAsync()
    {
        ...
        // v2.13.136 注册 UI 重构：删除 License / LicenseMessage / LicenseRegInt 属性与 OnPostLicenseRegisterAsync handler
        // 注册验证统一在托盘端 LicenseForm 完成；Web 端 Settings/Index.cshtml 仅显示纯文本徽章
        // 全局拦截由 LicenseReadOnlyMiddleware 强制 POST/PUT/DELETE → 403
    }

    // 注意：数据库连接配置保存已迁移到 /api/v1/system/dbconfig/save (v2.13.19)
    // 注意：v2.13.136 注册授权写入/取消/清除已全部迁移到托盘端 LicenseForm（Web 端 0 写入调用）
